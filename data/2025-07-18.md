...
Motivation: Zak-OTFS在时延多普勒域（DD域）中信号处理具有可预测性，但仍需周期性导频传输。本文旨在消除这一需求。

Method: 利用DD域信道的预测能力，将检测到的数据符号作为导频，通过前一时刻的信道估计检测下一时刻的数据，实现差分通信。

Result: 方案提高了数据符号的能量分配和频谱效率，与现有方案相比，在较低复杂度下实现了更低的误码率。
...
TL;DR: 提出了一种离线训练的神经网络设计方法，通过生成合成训练数据集，确保网络在新信道上的性能，无需实际信道信息或在线调参。
...
Method: 设计合成训练数据集的标准，并基于此提出基准设计，确保神经网络在不同信道配置下的鲁棒性。

Result: 神经网络在固定和可变延迟扩展的信道上均表现出鲁棒泛化能力，且与网络架构无关。

Conclusion: 离线训练的神经网络能够在不依赖实际信道信息的情况下，实现对新信道的鲁棒性能。
...
TL;DR: 提出了一种统一的深度学习框架，结合ResNet和注意力机制，通过时间域数据增强和Focal Loss解决生物信号分析和类别不平衡问题，在多个基准数据集上表现优异。
...
Method: 使用ResNet-CNN结合注意力机制，采用时间域数据增强和Focal Loss，预处理包括小波去噪、基线去除和标准化。

Result: 在UCI Seizure EEG、MIT-BIH Arrhythmia和PTB Diagnostic ECG数据集上分别达到99.96%、99.78%和100%的准确率。

Conclusion: 该框架在性能和计算效率上均表现出色，适用于低端或可穿戴设备部署。
...
TL;DR: 论文提出了一种基于多机器学习分类器一致投票的保守卫星选择策略，增强了ZSM定位方法，显著提高了城市GNSS环境中的定位可靠性。


<details>
  <summary>Details</summary>
Motivation: 城市环境中，建筑物导致的信号遮挡和多径效应会严重影响GNSS定位精度，需改进现有方法以提高可靠性。

Method: 使用RF、GBDT和SVM三种分类器对GPS信号进行LOS/NLOS分类，仅在所有分类器一致且置信度超过阈值时才选择卫星用于定位。

Result: 实验表明，该方法显著提高了定位成功率和接收器包含率，尽管卫星数量减少导致定位范围略有增加。

Conclusion: 提出的方法在城市GNSS环境中有效提升了定位可靠性，验证了其实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在联合感知与通信优化中缺乏全局最优性或需要额外处理步骤。
...
TL;DR: 提出了一种基于AFDM的多模式索引调制方案（MM-AFDM-IM），通过动态选择星座模式和激活子载波提升频谱和能量效率。
...
Method: 设计多模式索引调制方案，动态选择星座模式和激活子载波，无需额外能耗。

Result: 仿真结果显示，MM-AFDM-IM在误码率上优于传统方案。

Conclusion: MM-AFDM-IM是一种高效且性能优越的调制方案，适用于高移动性通信。
...
TL;DR: 本文探讨了无调制可见光定位（uVLP）技术，分析了其与传统VLP的差异，并分类了现有uVLP技术。同时提出了分类框架，并讨论了当前挑战和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 传统VLP技术因调制LED的成本和复杂性限制了其广泛应用，uVLP通过利用未调制的光信号提供了一种低成本、高效的替代方案。

Method: 本文分析了uVLP的基本原理，比较了uVLP与传统VLP，并根据接收器技术分类了uVLP方法，提出了分类框架。

Result: uVLP提供了一种低成本、高效的室内定位方案，但仍需解决技术挑战以实现广泛应用。

Conclusion: uVLP是室内定位的有前景技术，未来研究应关注其鲁棒性和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 均匀线性阵列只能估计方位角，而UCA能提供360度方位角和俯仰角信息，因此需要一种简单高效的DOA估计方法。

Method: 通过角度量化，利用协方差计算和比较获取量化方位角估计，再通过显式公式计算俯仰角估计。

Result: 数值结果表明，该方法能有效估计方位角和俯仰角，并可作为高精度多维搜索的起点。
...
TL;DR: 提出了一种基于多端口S参数的模型，用于预测多频操作结构的散射特性，适用于非周期性配置和非数字调制。


<details>
  <summary>Details</summary>
Motivation: 现有模型难以准确预测多频操作结构的散射行为，特别是在空间时间调制或动态负载控制下。

Method: 扩展了多端口S参数模型，纳入结构散射、互耦合和非数字调制，适用于时间调制的多端口结构。
...
Method: 使用FCN估计重叠事件数量，结合CS模块重构信号成分，恢复单细胞特征。

Result: 比传统算法恢复更多事件（21%），分类准确率超过97%。
...
### [11] [Robust Resource Allocation for Pinching-Antenna Systems under Imperfect CSI](https://arxiv.org/abs/2507.12582)
*Ming Zeng,Xianbin Wang,Yuanwei Liu,Zhiguo Ding,George K. Karagiannidis,H. Vincent Poor*

Main category: cs.IT

TL;DR: 论文研究了多用户夹持天线系统中用户位置不确定时的资源分配问题，旨在最小化总发射功率并满足个体中断概率约束。


<details>
  <summary>Details</summary>
Motivation: 现有夹持天线设计假设用户位置完全已知，这在现实中不切实际，因此需要研究位置不确定时的资源分配方案。

Method: 针对单用户情况，结合几何分析和二分法优化天线位置和功率分配；多用户情况下，采用粒子群优化（PSO）算法处理非凸、不可微的优化问题。

Result: 仿真结果表明，所提方案优于传统固定天线系统，PSO天线放置策略在位置不确定时有效。

Conclusion: 夹持天线技术在位置不确定时仍能显著提升系统性能，PSO算法是解决非凸优化问题的有效方法。
...
### [12] [Learning-Based Interface for Semantic Communication with Bit Importance Awareness](https://arxiv.org/abs/2507.12850)
...
Method: 采用可训练参数的接口设计，并引入重要性感知网络动态适应信道带宽比和时变条件。
...
### [13] [Robust Beamforming Design for Secure Near-Field ISAC Systems](https://arxiv.org/abs/2507.12881)
*Ziqiang CHen,Feng Wang,Guojun Han,Xin Wang,Vincent K. N. Lau*

Main category: cs.IT

TL;DR: 研究了近场安全集成感知与通信（ISAC）系统中的鲁棒波束成形设计，考虑了信道不确定性约束，提出了一种低复杂度的次优解决方案。


<details>
  <summary>Details</summary>
Motivation: 近场ISAC系统在多用户、多目标和多窃听者场景下，需要同时满足感知波束增益、通信质量和安全性需求，现有方法在复杂性和性能上存在不足。

Method: 采用S-Procedure处理信道不确定性约束，将其转化为线性矩阵不等式（LMIs），并利用SROCR方法解决秩一约束问题。

Result: 数值结果表明，所提方案在感知波束增益、通信质量和安全性方面优于现有SDR和其他基线方案。

Conclusion: 所提出的波束成形设计显著提升了近场ISAC系统的安全性和鲁棒性。
...
### [14] [Secure Pinching Antenna-aided ISAC](https://arxiv.org/abs/2507.13131)
*Elmehdi Illi,Marwa Qaraqe,Ali Ghrayeb*

Main category: cs.IT

TL;DR: 提出了一种基于夹持天线（PA）的安全集成感知与通信系统（ISAC）方案，优化PA位置和信号波束成形，以提升感知性能并确保通信保密性。


<details>
  <summary>Details</summary>
Motivation: 研究如何在双功能雷达通信（DFRC）基站中，通过优化PA位置和信号设计，同时实现通信保密和对恶意目标的检测。

Method: 优化PA位置，设计合法信号波束成形和人工噪声协方差矩阵，以最大化感知性能，同时满足保密性和总功率约束。

Result: 数值实验表明，该方案在照明功率上比传统PA方案提升3dB，比传统ISAC系统提升30dB。

Conclusion: 提出的PA辅助方案在感知性能和通信保密性上显著优于传统方案。
...
### [15] [A Framework of Distributed Source Encryption using Mutual Information Security Criterion and the Strong Converse Theorem](https://arxiv.org/abs/2507.13294)
...
Conclusion: 标准安全准则下的速率区域边界被成功验证，为分布式安全源编码提供了更可靠的依据。
...
### [16] [Analytical Optimization for Antenna Placement in Pinching-Antenna Systems](https://arxiv.org/abs/2507.13307)
*Zhiguo Ding,H. Vincent Poor*

Main category: cs.IT

TL;DR: 本文通过解析优化方法研究了夹持天线系统的天线位置优化问题，揭示了天线位置对系统性能的影响，并针对不同传输模式提出了最优天线位置的闭式解。
...
Method: 采用解析优化方法，针对正交多址（OMA）和非正交多址（NOMA）传输模式，分别推导了最优天线位置的闭式解。

Result: 对于用户公平导向的OMA传输，天线位置应有益于所有用户；对于贪婪分配的OMA传输，天线位置靠近最近用户；对于NOMA传输，天线位置靠近最近用户。

Conclusion: 解析优化方法揭示了天线位置选择的关键因素，为夹持天线系统设计提供了理论指导。
...
### [17] [Pathology-Guided Virtual Staining Metric for Evaluation and Training](https://arxiv.org/abs/2507.12624)
*Qiankai Wang,James E. D. Tweel,Parsin Haji Reza,Anita Layton*

Main category: eess.IV

TL;DR: 提出了一种新的病理感知图像相似性度量PaPIS，用于虚拟染色评估，优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统评估方法（如FR-IQA）无法捕捉病理相关特征，专家评审又主观耗时。
...
### [18] [InSight: AI Mobile Screening Tool for Multiple Eye Disease Detection using Multimodal Fusion](https://arxiv.org/abs/2507.12669)
...
<details>
  <summary>Details</summary>
Motivation: 早期筛查眼病至关重要，但在低收入和中等收入国家以及资源有限的环境中，医疗资源不足。InSight旨在通过AI技术解决这一问题。

Method: InSight采用三阶段流程：实时图像质量评估、疾病诊断模型和DR分级模型。创新包括多模态融合技术、预训练方法和多任务模型。

Result: 图像质量检查器准确率接近100%，多模态疾病诊断模型比仅使用图像的模型性能提升6%（BRSET）和4%（mBRSET）。

Conclusion: InSight在多种图像条件下表现稳健，诊断准确率高，且计算效率是五个独立模型的五倍。
...
### [19] [TRIQA: Image Quality Assessment by Contrastive Pretraining on Ordered Distortion Triplets](https://arxiv.org/abs/2507.12687)
...
### [20] [Pixel Perfect MegaMed: A Megapixel-Scale Vision-Language Foundation Model for Generating High Resolution Medical Images](https://arxiv.org/abs/2507.12698)
*Zahra TehraniNasab,Amar Kumar,Tal Arbel*

Main category: eess.IV

TL;DR: Pixel Perfect MegaMed是一种基于多尺度Transformer架构的视觉语言基础模型，用于生成1024x1024分辨率的医学图像，解决了传统GANs和VAEs在保留细粒度细节上的不足。


<details>
  <summary>Details</summary>
Motivation: 医学图像合成需要高分辨率和细节保留，传统方法难以满足临床诊断需求。

Method: 采用多尺度Transformer架构和视觉语言对齐技术，结合医学术语和成像模态，生成高分辨率医学图像。
...
### [21] [Unleashing Vision Foundation Models for Coronary Artery Segmentation: Parallel ViT-CNN Encoding and Variational Fusion](https://arxiv.org/abs/2507.12938)
...
Conclusion: 提出的框架有效解决了冠状动脉分割的挑战，为CAD诊断提供了可靠工具。
...
### [22] [Improving Diagnostic Accuracy of Pigmented Skin Lesions With CNNs: an Application on the DermaMNIST Dataset](https://arxiv.org/abs/2507.12961)
...
### [23] [From Variability To Accuracy: Conditional Bernoulli Diffusion Models with Consensus-Driven Correction for Thin Structure Segmentation](https://arxiv.org/abs/2507.12985)
...
<details>
  <summary>Details</summary>
Motivation: 眼眶骨CT图像分割在定制植入物中至关重要，但现有方法在模糊区域常产生不连续或欠分割结果。

Method: 使用条件伯努利扩散模型生成多个可能的分割结果，并通过共识驱动修正结合位置接近性、共识水平和梯度方向相似性来修正困难区域。
...
### [24] [fastWDM3D: Fast and Accurate 3D Healthy Tissue Inpainting](https://arxiv.org/abs/2507.13146)
*Alicia Durrer,Florentin Bieder,Paul Friedrich,Bjoern Menze,Philippe C. Cattin,Florian Kofler*

Main category: eess.IV

TL;DR: 本文提出了一种快速3D健康组织修复方法fastWDM3D，结合DDPM和GAN，采用方差保持噪声计划，显著提高了采样速度。


<details>
  <summary>Details</summary>
Motivation: 健康组织修复在肿瘤生长模型和图像配准中有重要应用，但现有DDPM方法采样速度慢。

Method: 结合DDPM与GAN，采用方差保持噪声计划，并应用于3D小波扩散模型（WDM3D）。

Result: fastWDM3D在BraTS测试集上SSIM为0.8571，MSE为0.0079，PSNR为22.26，仅需2个时间步，速度提升800倍。

Conclusion: fastWDM3D是一种快速且准确的健康组织修复方法，性能优于现有DDPM。
...
### [25] [SpectraLift: Physics-Guided Spectral-Inversion Network for Self-Supervised Hyperspectral Image Super-Resolution](https://arxiv.org/abs/2507.13339)
*Ritik Shah,Marco F. Duarte*

Main category: eess.IV

TL;DR: SpectraLift是一种自监督框架，通过融合低分辨率高光谱图像（LR-HSI）和高分辨率多光谱图像（HR-MSI），无需点扩散函数校准或真实高分辨率高光谱图像（HR-HSI），即可恢复高分辨率高光谱图像。


<details>
  <summary>Details</summary>
Motivation: 高分辨率高光谱图像（HSI）在遥感和医学成像中至关重要，但现有方法通常需要不切实际的校准数据或真实HR-HSI。

Method: SpectraLift使用多光谱图像的谱响应函数（SRF）训练轻量级多层感知机（MLP），通过合成低分辨率多光谱图像（LR-MSI）和LR-HSI进行自监督学习。
