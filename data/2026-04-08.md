...
Motivation: 现有的图信号排列熵及其衍生方法都基于香农熵和符号动力学，缺乏基于条件熵的度量方法。本文旨在探索如何将流行的样本熵有效定义于图信号，以表征复杂网络上的非线性动力学。

Method: 提出图信号样本熵（SampEnG），通过构建基于多跳邻域的拓扑感知嵌入，在连续嵌入状态空间中计算有限尺度的相关和，将经典样本熵从一维/二维信号（时间序列、图像）推广到图信号。

Result: 在合成和真实数据集（气象站、无线传感器监测、交通系统）上的实验验证了SampEnG在路径和网格上能恢复已知的非线性动力学特征。在交通流分析中，基于有向拓扑的SampEnG对自由流和拥堵之间的相变特别敏感，提供了与现有香农熵方法互补的信息。

Conclusion: SampEnG为分析图信号开辟了新途径，将样本熵和条件熵概念推广到各种网络数据，扩展了非线性分析的应用范围。
...
<details>
  <summary>Details</summary>
Motivation: 解决无线网络中图结构干扰下的约束遍历资源优化问题，传统迭代算法计算成本高，需要能快速生成高质量资源分配的方法。

Method: 训练扩散模型匹配专家条件分布：1) 使用原始-对偶专家算法生成训练样本；2) 将分配视为已知信道状态图上的随机图信号；3) 采用U-Net架构的图神经网络块，以信道状态和节点状态为条件；4) 推理时直接采样生成分配向量。

Result: 在功率控制案例中，生成的时间共享功率分配实现了近最优的遍历和速率效用和近可行的遍历最小速率，在网络状态间表现出强大的泛化和迁移能力。

Conclusion: 扩散模型能够有效学习专家资源分配策略，实现计算高效的近最优资源优化，在无线网络资源管理中具有实用价值。
...
TL;DR: 提出一种两阶段呼吸率估计方案，通过检测准静态信号片段来应对大规模随机身体运动的挑战
...
Method: 两阶段方案：第一阶段使用增强的深度神经网络框架结合动态蛇形卷积检测微多普勒频谱中的准静态片段；第二阶段仅使用与检测到的准静态片段位置一致的脊线部分来减轻RBM干扰

Result: 实验结果表明，该方案能在大规模RBM的正常场景下准确检测准静态片段，从而降低后续呼吸率估计的误差

Conclusion: 提出的两阶段方案有效解决了大规模随机身体运动下的呼吸率估计问题，通过准静态片段检测显著提高了估计精度
...
TL;DR: DDA-Net：一种用于稀疏跳频导频下TDD大规模MIMO信道估计的模型驱动3D深度展开网络，结合精确物理数据一致性和学习到的DDA域先验，显著提升信道重建性能。


<details>
  <summary>Details</summary>
Motivation: 在TDD大规模MIMO系统中，稀疏跳频导频下的信道估计面临挑战：每个快照仅捕获一个窄导频块，在频率上跳变，相邻快照间隔数十毫秒。有限窗口泄漏和离网效应削弱了理想的多普勒-延迟-角度（DDA）稀疏性，限制了传统稀疏恢复方法和缺乏显式结构化变换域模型的纯数据驱动方法。

Method: 提出DDA-Net，一种模型驱动的3D深度展开网络，用于联合多快照信道状态重建。该方法展开ADMM公式，具有精确闭式数据一致性更新（避免张量求逆），通过轻量级多普勒域去噪器学习先验，并使用延迟过采样减少基失配。

Result: 在QuaDRiGa UMa-NLOS场景中，DDA-Net在10 dB SNR下比最佳基线改善NMSE超过5 dB；在3GPP CDL-B信道的零样本测试中，在相同SNR下保持约1.5 dB的优势。消融研究表明，跨场景的窗口级3D处理是必要的，而多普勒参数化增加了分布内增益，并在仅使用20个目标域样本进行少量微调后，在场景转移下恢复了明显优势。

Conclusion: 将精确物理数据一致性与学习到的DDA域先验相结合，是稀疏跳频导频下信道状态获取的有效且样本高效的方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统缺陷检测方法在处理非平稳系统、脉冲激励响应时存在局限性，需要建立数学基础更坚实的框架来量化结构扰动对瞬态信号能量分布的影响

Method: 1) 在希尔伯特空间L2(R)中建模测量响应；2) 使用时频表示作为有界线性分析算子；3) 引入非线性能量集中指数ECI量化时频平面局部能量；4) 推导紧凑的脉冲多分辨率能量检测器IMRED

Result: 实验验证显示IMRED检测器AUC达到0.908，比全局傅里叶带能量测量和非优化小波带聚合提供更清晰的类别分离，能有效捕捉缺陷引起的结构差异
...
TL;DR: 该论文研究了在单波导和多波导场景下，采用夹持天线系统（PASS）进行语义通信的性能，通过交替优化算法优化功率分配和天线位置，在满足比特用户QoS要求的同时提高了语义频谱效率。


<details>
  <summary>Details</summary>
Motivation: 研究动机是在异构用户下行非正交多址（NOMA）系统中，通过夹持天线系统实现语义通信，在满足比特用户服务质量（QoS）要求和比特到语义解码顺序约束的同时，提高语义频谱效率。

Method: 方法包括：1）在单波导场景中，采用交替优化算法优化用户功率分配系数和夹持天线位置；2）在多波导场景中，使用单调优化和最小化-最大化方法解决波导功率分配子问题，通过迭代最大化下界代理函数获得非递减目标序列。

Result: 数值结果表明，基于NOMA的PASS系统在满足比特用户QoS要求的同时，相比传统固定天线系统提供了更高的语义频谱效率。多波导场景在严格条件下（更高比特用户QoS和更广覆盖范围要求）对创建可调无线信道更为有利。

Conclusion: 结论是夹持天线系统结合语义通信在NOMA框架下能够有效提高频谱效率，多波导配置在严格通信条件下表现更优，为未来无线通信系统设计提供了有前景的方向。
...
TL;DR: 提出基于大语言模型知识库的语义通信系统SC-LMKB，利用LLM生成能力丰富知识库，通过幻觉过滤和跨域融合机制提升性能


<details>
  <summary>Details</summary>
Motivation: 传统语义通信系统的知识库获取成本高且规模有限，而大语言模型具有广泛知识和生成能力，可以显著丰富语义通信的知识库

Method: 1) 设计LLM生成机制：源数据生成采用提示工程策略，信道数据生成采用交叉注意力对齐方法；2) 提出跨域融合编解码框架：包含幻觉过滤阶段和跨域融合阶段；3) 提出结合交叉熵损失和重建损失的联合训练目标

Result: 在三个跨模态检索任务上的实验表明，SC-LMKB相比传统语义通信系统性能提升72.6%，相比LLM-enabled语义通信系统性能提升90.7%

Conclusion: 提出的SC-LMKB系统有效利用LLM生成能力丰富知识库，通过幻觉过滤和跨域融合机制缓解LLM幻觉问题，显著提升语义通信性能
...
### [8] [Learned Elevation Models as a Lightweight Alternative to LiDAR for Radio Environment Map Estimation](https://arxiv.org/abs/2604.05520)
*Ljupcho Milosheski,Fedja Močnik,Mihael Mohorčič,Carolina Fortuna*

Main category: eess.SP

TL;DR: 提出一个两阶段框架，通过卫星RGB图像预测高程图，再结合天线参数进行无线环境地图估计，无需3D数据推理


<details>
  <summary>Details</summary>
Motivation: 下一代无线系统（如6G）在高频段运行，信号传播对环境因素敏感，需要准确的无线环境地图。现有方法需要昂贵的3D环境数据（如LiDAR点云），成本高、数据量大且易过时

Method: 两阶段框架：第一阶段从卫星RGB图像学习预测高程图；第二阶段将预测的高程图与天线参数一起输入到无线环境地图估计器中

Result: 相比仅使用图像的基线方法，RMSE提升高达7.8%，在相同输入特征空间下运行，推理时无需3D数据

Conclusion: 该方法为可扩展的无线环境建模提供了实用替代方案，消除了推理时对3D数据的需求
...
### [9] [Rotatable Antenna Enabled Multi-Cell Mixed Near-Field and Far-Field Communications](https://arxiv.org/abs/2604.05565)
*Yunpu Zhang,Changsheng You,Ruichen Zhang,Beixiong Zheng,Hing Cheung So,Dusit Niyato,Tony Q. S. Quek*

Main category: eess.SP

TL;DR: 该论文研究多小区混合近场-远场通信系统，利用可旋转天线技术通过优化波束成形和天线旋转角度来抑制复杂的混合场干扰，提升系统性能。


<details>
  <summary>Details</summary>
Motivation: 现有混合近场-远场通信研究仅限于单小区场景，而实际多小区系统中存在更复杂的跨小区混合场干扰，需要新的解决方案。

Method: 提出基于可旋转天线的多小区混合场通信系统，通过联合优化发射波束成形和天线旋转角度来最大化网络总速率。针对单用户和多用户场景分别设计了分析方法和双层优化算法。

Result: 对于单用户场景，通过菲涅尔积分获得闭式解，证明天线旋转能有效抑制跨小区混合场干扰；对于多用户场景，开发了基于半定松弛、逐次凸逼近和粒子群优化的高效算法。

Conclusion: 可旋转天线技术为多小区混合近场-远场通信提供了有效的干扰抑制手段，通过利用额外的空间自由度显著提升系统性能。
...
### [10] [A BEV-Fusion Based Framework for Sequential Multi-Modal Beam Prediction in mmWave Systems](https://arxiv.org/abs/2604.05668)
*Jiaming Zeng,Cunhua Pan,Haoyang Weng,Ruijing Liu,Hong Ren,Jiangzhou Wang*

Main category: eess.SP

TL;DR: BEV-Fusion框架通过鸟瞰图空间统一多模态传感器数据，用于毫米波系统中的波束预测，在车辆高速移动场景中显著提升预测准确性。


<details>
  <summary>Details</summary>
Motivation: 毫米波系统中的波束训练开销在高速车辆场景中尤为严重，需要有效的波束预测方法来减少训练开销。现有方法通常融合全局池化的一维特征，无法保留跨模态的几何结构和视觉语义密度。

Method: 提出BEV-Fusion框架：1）在鸟瞰图空间统一相机、LiDAR、雷达和GPS多模态数据；2）使用基于交叉注意力的相机到BEV模块生成BEV对齐的视觉特征，无需精确相机标定；3）使用时序Transformer聚合五步序列观测进行运动感知的波束预测。

Result: 在DeepSense 6G基准测试的场景32、33和34上，BEV-Fusion达到约87%的距离基础准确率（DBA），优于TransFuser基线方法。

Conclusion: BEV空间融合为感知辅助的波束预测提供了有效的空间抽象，能够保留跨模态几何结构和视觉语义密度，在毫米波车辆通信中具有显著优势。
...
### [11] [Brain-to-Speech: Prosody Feature Engineering and Transformer-Based Reconstruction](https://arxiv.org/abs/2604.05751)
*Mohammed Salah Al-Radhi,Géza Németh,Andon Tchechmedjiev,Binbin Xu*

Main category: eess.SP

TL;DR: 提出一种从颅内脑电图(iEEG)进行脑到语音合成的新方法，通过韵律特征工程和专用Transformer架构提升语音重建质量
...
Method: 开发了从复杂iEEG信号提取韵律特征(语调、音高、节奏)的管道，并设计了专门用于脑到语音任务的Transformer编码器架构，整合韵律特征以增强语音重建

Result: 在定量和感知指标上均优于传统基线方法(Griffin-Lim和CNN重建)，生成的语音具有更好的可懂度和表现力

Conclusion: 通过特征提取和Transformer学习的创新，推动了AI驱动的神经假体领域发展，为恢复言语障碍者通信铺平道路，未来可探索扩散模型和实时推理系统
...
TL;DR: 提出用于ISAC系统的反馈驱动配置调优框架，通过UE侧自适应调整传感参数，在动态条件下满足网络约束，提高传感可靠性和降低延迟
...
Method: 提出RACE CMA算法，集成两阶段竞赛、公共随机数、噪声感知排序和可行约束处理，通过阈值优化作为案例研究进行基准测试

Result: 传感可靠性提高约35%，计算成本降低约25%，性能成本效率提升约两倍

Conclusion: UE侧配置调优是在实际系统约束下增强闭环ISAC性能的有前景机制
...
TL;DR: 论文提出了一种结合几何离散化策略和扩展衍射模型的射线追踪方法，用于精确高效地模拟弯曲金属体的近场散射，特别适用于车辆传播和ISAC信道建模。


<details>
  <summary>Details</summary>
Motivation: 准确模拟弯曲金属体（如车辆和路边结构）的散射对于蜂窝和车辆信道建模以及雷达应用至关重要。现有方法通常用平面面片近似弯曲表面并结合射线追踪与衍射方法，但其精度严重依赖于几何离散化和衍射建模。

Method: 采用基于射线追踪的方法，结合均匀衍射理论（UTD），并扩展了顶点衍射和双反射相互作用。提出了一种将面片尺寸与局部曲率和波长关联的离散化策略，以平衡几何保真度、计算精度和效率。

Result: 验证表明，适当的离散化结合扩展衍射建模显著提高了弯曲体散射预测的准确性。该方法在标准几何体（球体和圆柱体）以及实际车辆模型上均表现出良好性能，为车辆传播和ISAC信道建模提供了计算高效的框架。

Conclusion: 提出的方法通过优化离散化策略和扩展衍射模型，有效提高了弯曲金属体散射建模的精度和效率，为实际车辆传播和ISAC信道建模提供了实用的解决方案。
...
TL;DR: 提出毫米波ISAC网络中的协作感知框架，通过非相干回波功率合并实现基站间协作，无需专用资源或感知结果交换，显著提升感知性能


<details>
  <summary>Details</summary>
Motivation: 解决毫米波ISAC网络中感知可靠性的挑战，特别是安全关键应用对高可靠性保证的需求，通过基站协作提升感知性能

Method: 提出协作感知框架：目标由最近基站感知，同时利用相邻基站的收发回波进行非相干功率合并；使用随机几何分析感知/通信覆盖和速率

Result: 结果显示感知性能大幅提升，通信损失有限；协作感知元分布分析表明高可靠性尾部改善，更多目标能够满足严格可靠性要求

Conclusion: 提出的协作感知框架能够显著提升毫米波ISAC网络的感知可靠性，特别适用于安全关键应用，为未来ISAC网络设计提供了有效方案
...
### [15] [CI-ICM: Channel Importance-driven Learned Image Coding for Machines](https://arxiv.org/abs/2604.05347)
*Yun Zhang,Junle Liu,Huan Zhang,Zhaoqing Pan,Gangyi Jiang,Weisi Lin*

Main category: eess.IV

TL;DR: 提出CI-ICM方法，通过通道重要性驱动优化机器视觉任务性能，在给定比特率约束下最大化目标检测和实例分割精度


<details>
  <summary>Details</summary>
Motivation: 传统以人类视觉为中心的图像压缩方法不适用于机器视觉任务，因为两者的视觉特性和特征特性不同。需要专门针对机器视觉的压缩方法。

Method: 1. 通道重要性生成模块量化通道重要性并排序；2. 特征通道分组缩放模块根据重要性非均匀分组并调整动态范围；3. 通道重要性上下文模块分配比特率；4. 任务特定通道适应模块增强多任务特征。

Result: 在COCO2017数据集上，相比基线编解码器，CI-ICM在目标检测上获得16.25%的BD-mAP@50:95增益，在实例分割上获得13.72%增益。消融研究验证了各模块有效性。

Conclusion: CI-ICM建立了机器视觉中心压缩的特征通道优化方法，弥合了图像编码与机器感知之间的差距，为机器视觉任务提供了高效的压缩解决方案。
...
### [16] [Generative Channel Knowledge Base With Environmental Information for Joint Source-Channel Coding in Semantic Communications](https://arxiv.org/abs/2604.05342)
...
Method: 1) 建立环境感知数据集，包含空间位置、图像特征、语义特征和信道矩阵；2) 设计ROI过滤算法去除无关语义成分；3) 开发Transformer生成框架学习环境信息到信道矩阵的映射；4) 提出CKB驱动的JSCC语义通信架构

Result: 多维特征融合方法实现10^-3级别的信道矩阵估计误差，CKB驱动的JSCC语义通信框架在传输性能上显著优于现有基准方案

Conclusion: 提出的生成式信道知识库能有效融合环境信息，提升语义通信系统的传输性能，为6G通信中的语义传输提供了新思路
...
### [17] [Non-GRS type MDS and AMDS codes from extended TGRS codes](https://arxiv.org/abs/2604.05682)
*Meiying Zhang,Shudi Yang,Yanbin Zheng*

Main category: cs.IT

TL;DR: 本文构造了一类扩展扭曲广义Reed-Solomon码，确定了其成为MDS或AMDS码的充要条件，证明了它们与广义Reed-Solomon码不等价，并计算了特定情况下的覆盖半径和深洞。


<details>
  <summary>Details</summary>
Motivation: MDS和AMDS码因其代数特性和优异的纠错能力，在通信系统、数据存储和量子编码等领域广泛应用。本文旨在构造一类扩展扭曲广义Reed-Solomon码，研究其成为MDS或AMDS码的条件，并探索其与经典广义Reed-Solomon码的区别。

Method: 构造了一类扩展扭曲广义Reed-Solomon码，通过代数方法分析了这些码的结构特性，推导了其成为MDS或AMDS码的充要条件，并证明了它们与广义Reed-Solomon码不等价。

Result: 确定了扩展扭曲广义Reed-Solomon码成为MDS或AMDS码的充要条件，证明了这些码与广义Reed-Solomon码不等价，并在特定情况下计算了这些码的覆盖半径和深洞。

Conclusion: 本文成功构造了一类具有良好代数特性的扩展扭曲广义Reed-Solomon码，为其在纠错编码领域的应用提供了理论基础，特别是在需要MDS或AMDS特性的场景中具有重要价值。
...
### [18] [The Incidence-Multiplicity Bound for Linear Exact Repair in MDS Array Codes](https://arxiv.org/abs/2604.05692)
*Huawei Wu*

Main category: cs.IT

TL;DR: 该论文研究了MDS阵列码的线性精确修复问题，提出了更紧的修复带宽下界（关联多重性界），并证明该下界在广泛参数范围内是可达的。


<details>
  <summary>Details</summary>
Motivation: 现有基于投影计数的方法给出的修复带宽下界在r≥3且ℓ≥2时无法达到，需要寻找更紧的下界来更好地理解MDS阵列码的精确修复极限。

Method: 通过改进计数论证方法，建立了关联多重性下界。使用域约简技术和正规有理曲线构造达到该下界的MDS阵列码。

Result: 证明了对于任意(n,k,ℓ) MDS阵列码，平均和最坏情况下的修复带宽及I/O至少为ℓ(n-1)-(r-1)(q^ℓ-1)/(q-1)。该下界在广泛参数范围内可达。

Conclusion: 关联多重性是超越两奇偶校验情况的MDS阵列码线性精确修复的支配几何原理，为修复带宽优化提供了理论基础。
...
### [19] [Foundations of Future Communication Systems: Innovations in Communication - A Report](https://arxiv.org/abs/2604.05694)
*Christian Deppe,Eduard Jorswieck,Pin-Hsun Lin,Vida Gholamian,Marcel A. Mross*

Main category: cs.IT

TL;DR: FFCS会议汇集了信息论、量子通信、分子通信、语义通信和安全网络设计等领域的研究者，探讨下一代通信系统的基础原理，涵盖从经典香农理论到后香农范式、量子信息科学和新兴物理通信模型的研究。


<details>
  <summary>Details</summary>
Motivation: 为下一代通信系统建立理论基础，超越传统的速率中心视角，在现实的物理、架构和安全约束下重新审视基础极限，促进跨学科交流与合作。

Method: 通过会议平台汇集邀请报告、投稿报告和海报展示，涵盖识别通信、纠缠辅助网络、语义和目标导向通信、分子和纳米系统编码、安全认证机制等研究方向。

Result: 会议展示了当代通信研究的广度，包括后香农范式、量子信息科学、物理层通信模型等前沿方向，强调可靠性、识别、语义、资源效率和信任等新维度。

Conclusion: FFCS会议报告记录了当前通信研究的前沿领域，旨在提供结构化概述并促进理论和实验领域的持续合作，为未来通信范式的发展奠定基础。
...
### [20] [Near-Field Integrated Sensing, Computing and Semantic Communication in Digital Twin-Assisted Vehicular Networks](https://arxiv.org/abs/2604.05797)
*Yinchao Yang,Yahao Ding,Jiaxiang Wang,Zhaohui Yang,Chen Zhu,Zhaoyang Zhang,Dusit Niyato,Mohammad Shikh-Bahaei*

Main category: cs.IT

TL;DR: 提出集成感知、计算和语义通信框架，用于近场数字孪生车辆网络，通过语义通信和毫米波雷达实现高效数据传输与精准环境感知。


<details>
  <summary>Details</summary>
Motivation: 数字孪生技术在车辆网络中具有变革潜力，但在动态环境中面临大规模数据传输、精准感知和严格计算约束等挑战，需要新的解决方案。

Method: 采用多用户MIMO配置，路边单元使用语义通信服务车辆，同时利用毫米波雷达进行环境映射；通过粒子滤波实现高精度车辆跟踪；提出混合启发式算法进行车辆-RSU分配，以及交替优化方法确定语义提取比率和波束成形矩阵。

Result: 数值结果表明，所提出的ISCSC框架在受限资源条件下，传输速率提高了20%，同时保持了现有ISAC方案的感知精度。

Conclusion: 该集成框架有效解决了数字孪生车辆网络中的同步挑战，通过语义通信和感知计算的协同优化，在资源受限条件下实现了性能提升。
...
### [21] [Improved Capacity Upper Bounds for the Deletion Channel using a Parallelized Blahut-Arimoto Algorithm](https://arxiv.org/abs/2604.05867)
*Martim Pinto,João Ribeiro*

Main category: cs.IT

TL;DR: 通过GPU并行化优化Blahut-Arimoto算法，获得二进制删除信道容量的改进上界，证明当删除概率d≥0.64时，容量上限为0.3578(1-d)。


<details>
  <summary>Details</summary>
Motivation: 二进制删除信道的容量界限问题长期以来是信息论中的重要开放问题，现有方法计算复杂，需要更高效的计算技术来获得更精确的容量上界。

Method: 通过GPU并行化技术优化Blahut-Arimoto算法，利用GPU的大规模并行计算能力加速容量上界的计算过程。

Result: 获得了二进制删除信道容量的改进上界，具体证明当删除概率d≥0.64时，容量上限为0.3578(1-d)。

Conclusion: GPU并行化为计算信道容量界限提供了有效工具，显著提升了计算效率，为二进制删除信道容量问题提供了新的上界结果。
...
### [22] [A Tensor-Train Framework for Bayesian Inference in High-Dimensional Systems: Applications to MIMO Detection and Channel Decoding](https://arxiv.org/abs/2604.05890)
*Luca Schmid,Dominik Sulz,Shrinivas Chimmalgi,Laurent Schmalen*

Main category: cs.IT

TL;DR: 提出基于张量链(TT)格式的贝叶斯推理框架，用于解决高维离散输入加性噪声模型的推理问题，通过低秩TT表示实现紧凑存储和高效计算。


<details>
  <summary>Details</summary>
Motivation: 高维离散输入加性噪声模型中的贝叶斯推理面临计算挑战，因为联合后验概率质量函数的支持随未知变量数量呈指数增长，需要寻找可处理的推理方法。

Method: 提出张量链(TT)框架，利用联合对数后验概率质量函数在TT格式中具有精确低秩表示的特性。开发实用推理程序，使用TT-cross算法初始化截断泰勒级数来近似对数后验的指数，以恢复符号级后验边际。

Result: 数值结果显示，在广泛的信噪比范围内实现了接近最优的误码率性能，同时仅需要适度的TT秩。为两个典型通信问题推导了显式低秩TT构造：AWGN下的线性观测模型（应用于MIMO检测）和二进制输入AWGN信道上的二进制线性分组纠错码的软判决解码。

Conclusion: 张量链框架为离散输入加性噪声模型提供了可处理、接近最优的贝叶斯推理方法，展示了张量网络方法在通信系统中高效贝叶斯推理的潜力。
...
### [23] [Covering-radius and Collinearity- Minimizing Pilots for Channel Estimation in TDD Systems](https://arxiv.org/abs/2604.06041)
...
<details>
  <summary>Details</summary>
Motivation: 在正交频分复用时分双工系统中，需要在连续子带和公平性约束下设计导频模式，以有效利用最近时隙的延迟-多普勒稀疏性，提升最新时隙恢复性能。

Method: 提出几何感知的时频联合导频分配方法，基于最小覆盖半径和共线性控制原则构建混合整数优化模型，平衡网格覆盖和冗余共线性抑制，并采用对称性避免细化策略。

Result: 数值结果表明，MCC导频模式在代理几何度量和最新时隙恢复性能方面均有显著提升，优于传统导频设计方法。

Conclusion: 在滑动窗口最新时隙恢复框架下，有效的导频设计应平衡网格覆盖和共线性控制，MCC模式为OFDM-TDD系统提供了实用的导频分配解决方案。
...
### [24] [Singular Relative Entropy Coding with Bits-Back Rejection Sampling](https://arxiv.org/abs/2604.06055)
*Gergely Flamich,Spencer Hill*

Main category: cs.IT

TL;DR: 提出BBRS算法，结合bits-back编码和拒绝采样，实现相对熵编码，相比现有方法更简单高效


<details>
  <summary>Details</summary>
Motivation: 相对熵编码通常只能达到互信息下界加对数因子，现有方法复杂且不实用，需要更简单高效的实现

Method: 结合bits-back编码和贪婪拒绝采样，构建bits-back拒绝采样器(BBRS)

Result: BBRS达到与Sriramu和Wagner方法相同的渐近效率，但分析更简单、常数更好，且可用标准方法实现

Conclusion: BBRS为相对熵编码提供了实用高效的解决方案，特别适用于奇异信道
...
### [25] [Multilevel Coset Codes on Lattices](https://arxiv.org/abs/2604.06125)
*Leopold Bertholet,Chloe Makdad,Stephen Mackes,Daniel Chew,Matthew Robinson*

Main category: cs.IT

TL;DR: 提出了一种新颖的coset Bombe码，将极化码推广到密集格结构，在AWGN信道中显著优于现有BICM和MLC方案，获得0.8dB增益并减少一半块延迟。


<details>
  <summary>Details</summary>
Motivation: 现有编码方案在密集格调制（如16-QAM）中性能有限，需要结合密集格的几何优势与极化码的容量逼近特性，以提升AWGN信道中的性能。

Method: 采用多级编码与非二进制码设计格调制，利用Voronoi成形技术，将极化码推广到密集格结构（如D4格），形成coset Bombe码。

Result: 在AWGN信道中，coset Bombe码在16-QAM上显著优于BICM和MLC方案，获得高达0.8dB增益，块延迟减少一半，在256和1024比特码字上保持优越的BER/BLER性能。

Conclusion: coset Bombe码成功将极化码推广到密集格结构，结合了几何优势与容量逼近特性，为格调制提供了高性能编码方案。
