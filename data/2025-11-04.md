...
TL;DR: 提出了一种基于Turbo结构的深度语义自动编码器(Turbo-DSA)，通过端到端设计实现语义级联合信源信道编码，在低信噪比条件下显著提升海洋物联网的语义传输效率和环境适应性。


<details>
  <summary>Details</summary>
Motivation: 随着海洋经济和海事活动的快速发展，当前海洋设备在数据传输效率和语义理解方面存在不足，需要提升海洋物联网的语义传输能力。

Method: 采用transformer技术构建语义编码器和解码器，将消息转换为语义向量；利用Turbo结构分离语义向量确保精确传输，并通过Turbo迭代解码优化语义向量表示；通过目标函数、语义提取和训练过程的整体设计实现transformer与Turbo结构的深度融合。

Result: 相比传统Turbo编码技术，Turbo-DSA具有更快的收敛速度；在双语评估替代分数和句子相似度等关键性能指标上超越现有基准方法，在低信噪比条件下表现出优越的文本语义传输效率和海洋信道环境适应性。

Conclusion: Turbo-DSA通过transformer与Turbo结构的深度融合，有效解决了海洋物联网中的语义传输效率问题，为海洋环境下的可靠语义通信提供了创新解决方案。
...
TL;DR: 本文分析了分布式假设检验场景中，当决策中心有本地观测时，传感器通过多址信道传输信息的Stein指数特性。研究发现在某些条件下，传感器通信对Stein指数没有帮助。


<details>
  <summary>Details</summary>
Motivation: 研究在分布式假设检验中，当决策中心有本地观测时，传感器通过多址信道传输信息是否能够提升检测性能（以Stein指数衡量）。

Method: 通过分析多址信道特性和成本约束条件，推导Stein指数的上下界，特别关注信道是否完全连通的情况。

Result: 对于加性噪声MAC和完全连通的离散多址信道，传感器通信对Stein指数无提升；对于非完全连通信道，Stein指数更高且等于零速率无噪声通信链路的性能。

Conclusion: 在某些信道条件下，分布式假设检验中的传感器通信对Stein指数没有贡献，决策中心的本地观测已足够；而在非完全连通信道中，通信能显著提升性能。
...
TL;DR: 本文研究了扭曲广义Reed-Solomon(TGRS)码的解码问题，提出了两种更高效的解码算法：一种适用于通用TGRS码，另一种专门针对几乎MDS TGRS码。


<details>
  <summary>Details</summary>
Motivation: 现有的TGRS码解码算法在性能上不够高效，需要开发更优化的解码方法来提高解码效率。

Method: 首先精确描述了TGRS码的关键方程，然后提出了通用TGRS码的解码算法；其次专门针对几乎MDS TGRS码开发了另一种解码算法。

Result: 所提出的两种解码算法在性能上分别优于Sun等人(2024)和Sui等人(2023)提出的算法，具有更高的解码效率。

Conclusion: 本文成功开发了两种高效的TGRS码解码算法，为TGRS码的解码提供了更优的解决方案。
...
TL;DR: 本文通过初等方法研究了有限域上ω-权重的MacWilliams扩展性质和常重码，当ω为常数1映射时，结果可恢复Hamming度量码的两个经典结论。


<details>
  <summary>Details</summary>
Motivation: 研究有限域上ω-权重的MacWilliams扩展性质和常重码，为Hamming度量码的经典结果提供更一般的推广。

Method: 采用初等线性代数方法和基于双计数论证推导出的ω-权重子空间的两个关键恒等式。

Result: 建立了ω-权重下的MacWilliams扩展性质，并证明了常重码的结构特征。当ω为常数1时，结果可恢复Hamming度量码的两个经典定理。

Conclusion: 该初等方法成功推广了Hamming度量码的经典结果到更一般的ω-权重情形，为编码理论提供了新的分析工具。
...
TL;DR: 本文研究了天地一体化通信系统中的公平性设计问题，提出了一种基于负载均衡框架的关联模式优化方法，并设计了遗传算法来解决大规模网络中的组合优化问题。


<details>
  <summary>Details</summary>
Motivation: 天地一体化通信系统需要在大范围区域内提供普适服务，但传统方法难以在异构接收器（包括接入点和卫星）环境下保证用户间的公平性，特别是在考虑不完美信道状态信息的情况下。

Method: 推导了任意关联模式下每个用户的上行链路数据传输的遍历吞吐量，建立了通用公平性优化问题，并针对大规模网络设计了基于遗传算法的低计算复杂度求解方法，同时结合了发射功率控制。

Result: 数值结果表明，用户关联模式对网络吞吐量有显著影响，提出的遗传算法在小规模网络中与穷举搜索性能相同，在大规模网络中能发现实用的关联模式，负载均衡结合功率控制相比传统方案显著提升了系统性能。

Conclusion: 基于遗传算法的负载均衡方法能有效解决天地一体化通信系统中的公平性问题，在保证性能的同时降低了计算复杂度，为大规模网络部署提供了可行的解决方案。
...
TL;DR: HyRES是一种混合存储方案，结合了数据复制和纠删码的最佳特性，在存储成本、文件丢失概率和修复流量方面都优于传统方案。
...
Method: 提出HyRES混合方案，综合考虑存储网络规模的影响，并通过模拟验证理论结果。

Result: HyRES相比纯复制方案降低存储成本，相比两种传统方案降低文件丢失概率，在网络规模考虑下甚至比复制方案降低修复流量。

Conclusion: HyRES方案在多个性能指标上优于传统存储方案，提供了更好的设计灵活性和潜在性能。
...
TL;DR: 本文提出了使用线性代数框架的MDS可转换码带宽成本的新下界，改进了某些参数范围内的先前结果，并在特定条件下与现有构造匹配，证明了下界的紧性。


<details>
  <summary>Details</summary>
Motivation: 研究MDS可转换码的带宽成本下界，以改进现有理论结果并为实际编码方案提供理论指导。

Method: 采用线性代数框架推导MDS可转换码的带宽成本下界。

Result: 新下界在特定参数范围内改进了先前结果，当r^F ≤ r^I ≤ k^F时与Maturana和Rashmi的构造匹配，证明了下界的紧性。

Conclusion: 提出的线性代数框架能够推导出紧的带宽成本下界，为MDS可转换码的设计提供了理论依据。
...
TL;DR: 本文研究了分布式多RIS辅助MIMO通信系统中基于自动编码器的对抗攻击与防御，分析了RIS数量与系统性能的关系，并提出基于对抗训练的防御机制。


<details>
  <summary>Details</summary>
Motivation: 当前无线通信系统广泛采用机器学习和深度学习技术进行优化，但对其在对抗攻击下的脆弱性关注不足，特别是在分布式RIS辅助的MIMO系统中。

Method: 建立了分布式多RIS信道传播模型，推导了聚合信道的闭式统计信息，使用符号错误率评估系统性能，并提出了基于对抗训练的防御机制。

Result: 增加RIS数量可降低系统SER，但在白盒攻击场景下会使对抗攻击更具破坏性；提出的防御方法能显著减轻攻击影响，并在无攻击时进一步降低SER。

Conclusion: 所提出的防御方法在对抗攻击下表现出强鲁棒性，且在多普勒引起的信道变化下仍能保持性能稳定。
...
Result: BCJRFormer在二进制和四进制单消息传输中达到与BCJR算法相当的误码率，且计算复杂度与噪声副本数量呈二次方关系，显著优于指数复杂度。

Conclusion: 构建了高效且性能优异的端到端基于Transformer的解码流水线，为DNA数据存储等需要处理多个噪声副本的应用提供了可行解决方案。
...
TL;DR: 本文研究了Levenshtein序列重构问题，确定在二进制序列和最多删除t个符号的情况下，重构大小为ℓ-1的候选序列列表所需的最小信道输出数量。


<details>
  <summary>Details</summary>
Motivation: 解决在删除错误信道中，如何通过有限数量的不同信道输出来重构原始传输序列的问题，这对于可靠通信具有重要意义。

Method: 通过分析ℓ≥3个删除球（半径为t）的交集最大可能大小，其中这些球中心是不同的二进制序列，且满足n≥t+ℓ-1和t≥1的条件。

Result: 确定了在给定条件下，重构候选序列列表所需的最小信道输出数量，这对应于删除球交集的最大可能大小。

Conclusion: 该研究为删除信道中的序列重构问题提供了理论界限，对编码理论和可靠通信系统设计具有重要价值。
...
TL;DR: 本文研究流体集成反射发射表面(FIRES)，这是一种能够同时控制传输和反射、相位及几何定位的智能表面。通过覆盖中心系统模型分析了两用户下行链路场景，推导了覆盖范围边界，并提出了覆盖最大化的双层优化框架。


<details>
  <summary>Details</summary>
Motivation: 传统的同时传输和反射可重构智能表面(STAR-RIS)在覆盖范围方面存在局限，需要开发能够联合利用几何重定位和被动能量控制的新型智能表面来提升性能。

Method: 开发了覆盖中心系统模型，推导了远场视距覆盖边界闭式解，提出了包含外层元素位置搜索和内层资源分配的双层优化框架，支持OMA和NOMA两种多址接入方案。

Result: 仿真显示FIRES相比传统STAR-RIS显著扩大了覆盖区域，NOMA在可行时能带来额外覆盖增益，分析边界与仿真结果匹配良好，FIRES对相位控制缺陷具有鲁棒性。

Conclusion: FIRES通过联合利用几何重定位和被动能量控制，在相同元件预算下显著优于传统STAR-RIS，为智能表面设计提供了新的优化维度。
...
TL;DR: 本文提出了适用于分布式矩阵乘法的代数函数域构造，为多项式码和Matdot码提供最优恢复阈值，通过有理函数域的扩展实现。


<details>
  <summary>Details</summary>
Motivation: 将多项式码和Matdot码扩展到代数函数域的主要挑战在于构造最优解码方案，需要解决固定矩阵乘法的最优恢复阈值问题。
...
TL;DR: 提出基于条件扩散模型的场景特定信道生成方法，用于增强神经接收器的训练，在叠加导频传输系统中显著提升性能


<details>
  <summary>Details</summary>
Motivation: 神经接收器在无线通信系统中表现优异，但需要大量场景特定的信道数据进行训练，这些数据在实际中难以获取

Method: 使用条件扩散模型根据用户位置和速度信息生成高质量信道数据，用于数据增强来训练基于叠加导频传输的神经接收器

Result: 该方法生成高保真信道样本，显著提升神经接收器在目标场景中的性能，优于传统数据增强和基于生成对抗网络的技术

Conclusion: 基于条件扩散模型的信道生成方法能有效解决训练数据不足问题，为神经接收器提供高质量的数据增强方案
...
TL;DR: 本文从信息论角度提出了LLMs的语义信息理论框架，将token作为基本单位，定义了预训练中的定向率失真函数、后训练中的定向率奖励函数以及推理阶段的语义信息流等结构无关的信息论度量。
...
Method: 基于率失真函数理论、定向信息和格兰杰因果理论，构建LLMs的语义信息理论，定义概率模型和结构无关的信息论度量，并深入探讨token级语义嵌入和最优向量化方法。

Result: 提出了通用的自回归LLM定义，从理论上推导了Transformer架构的性能指标（如ELBO、泛化误差界、记忆容量和语义信息度量），并将Mamba/Mamba2和LLaDA等架构纳入框架。
...
<details>
  <summary>Details</summary>
Motivation: DNA分子标记是DNA可视化和分析的基础技术，需要开发能够纠正标记过程中可能出现的错误的编码方案。

Method: 建立数学模型，为标记DNA序列开发纠错码，构建显式系统编码器来处理单替换、插入和删除错误，研究两种不同的标记集配置。

Result: 建立了标记DNA序列的纠错码界限，并成功构建了针对单错误的系统编码器。

Conclusion: 提出的纠错码方案能够有效保护标记DNA序列免受单替换、插入和删除错误的影响，为DNA标记技术提供了可靠的错误纠正保障。
...
TL;DR: 本文针对Ding等人提出的九个最优三元循环码问题中的第9个问题，通过确定有限域上特殊多项式的根集，给出了不完全解答，并基于这些多项式构造了两类满足Sphere Packing Bound的最优三元循环码。


<details>
  <summary>Details</summary>
Motivation: 循环码是线性码的子类，在消费电子、数据存储和通信系统中具有重要应用，因其高效的编码和解码算法。Ding等人在2013年提出了九个关于最优三元循环码的开放问题，其中第9个问题尚未完全解决。

Method: 通过确定有限域上特殊多项式的根集，对第9个问题给出不完全解答，并基于这些特殊多项式构造最优三元循环码。

Result: 成功构造了两类满足Sphere Packing Bound的最优三元循环码，为第9个问题提供了部分解决方案。

Conclusion: 本文对Ding等人提出的第9个最优三元循环码问题给出了不完全解答，并基于有限域上的特殊多项式构造了两类最优三元循环码，推动了该领域的研究进展。
...
TL;DR: 构建了有限域F2上的投影三权重线性码和两类投影四权重线性码，通过加性特征确定其权重分布，部分码可用于秘密共享方案


<details>
  <summary>Details</summary>
Motivation: 研究具有特定权重分布的投影线性码，探索其在秘密共享等密码学应用中的潜力

Method: 使用定义集构造方法，通过加性特征分析确定码的权重分布

Result: 成功构造了一类投影三权重线性码和两类投影四权重线性码，并确定了它们的权重分布

Conclusion: 所构造的投影三权重线性码和一类四权重线性码（定理4.1）适用于秘密共享方案
...
TL;DR: 证明了存在图灵机能够为任意离散无记忆信道构造容量可达码，给定信道转移概率、目标速率和误差容限，机器通过穷举搜索找到满足要求的码。


<details>
  <summary>Details</summary>
Motivation: 从算法角度研究容量可达码的构造问题，探讨在可计算实数框架下信道编码的可行性。

Method: 基于香农信道编码定理，采用穷举搜索方法系统枚举递增块长的所有码，直到找到有效码。使用递归函数理论形式化构造过程。

Result: 构造了一个μ-递归函数FindCode，在目标速率小于信道容量时输出有效码的编码。通过Kleene范式定理证明图灵机可解性。

Conclusion: 证明了在可计算实数假设下，容量可达码的构造问题是图灵可解的，且该假设无法进一步弱化。
...
TL;DR: 本文提出了使用嵌套数η(ℋ)作为新的结构参数来推导PICOD问题最优长度的下界，该下界在某些特殊结构的PICOD问题中能够提供紧致的界限。


<details>
  <summary>Details</summary>
Motivation: 在可塑性索引编码(PICOD)中，寻找最优编码长度和设计短长度编码方案是核心问题。需要开发新的结构参数来更好地理解PICOD问题的复杂性。

Method: 引入了一个新的结构参数——嵌套数η(ℋ)，该参数与表示PICOD问题的超图ℋ相关联，用于推导最优PICOD长度的下界。

Result: 嵌套数下界虽然不比已知下界更强，但具有计算优势。对于某些特殊结构的PICOD问题，基于嵌套数的下界是紧致的。

Conclusion: 嵌套数作为新的结构参数为PICOD问题提供了有用的下界分析工具，特别在计算效率和特殊结构问题中表现出优势。
...
TL;DR: 本文分析了可编程无线环境中夹持天线系统的性能，考虑了实际实现中的离散位置限制，开发了分析框架评估双状态系统的速率性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究假设夹持天线位置可连续重构，忽略了实际实现中只能有有限数量天线位置的离散性限制。

Method: 开发了分析框架评估双状态夹持天线系统的速率性能，考虑了波导的离散空间结构，推导了遍历可实现数据速率的闭式表达式。

Result: 仿真结果表明，使用有限数量的夹持天线即可实现接近连续配置的性能。

Conclusion: 该研究为可编程无线环境中夹持天线系统的设计和可扩展性提供了有价值的见解。
...
TL;DR: 本文提出了一种基于级联Reed-Solomon码和Hadamard码的噪声弹性向量符号架构，通过解决直方图恢复问题实现了高效的信息检索。


<details>
  <summary>Details</summary>
Motivation: 随机线性码在向量符号架构中难以在噪声环境下解码，限制了信息恢复能力。需要一种既保持准正交性又具备噪声弹性的编码方案。

Method: 使用Reed-Solomon码和Hadamard码的级联编码，提出直方图恢复问题，并利用列表解码相关算法提供最优解。

Result: 开发出具有形式化保证的噪声弹性VSA，在高效编码、准正交性和恢复能力方面优于Hadamard码等类似方案。

Conclusion: 该方法在不依赖启发式方法或训练的情况下，实现了改进参数的噪声弹性向量符号架构，为神经符号AI系统提供了可靠的信息表示技术。
...
### [22] [Towards Reliable Pediatric Brain Tumor Segmentation: Task-Specific nnU-Net Enhancements](https://arxiv.org/abs/2511.00449)
*Xiaolong Li,Zhi-Qin John Xu,Yan Ren,Tianming Qiu,Xiaowen Wang*

Main category: eess.IV

TL;DR: 本文针对儿科脑肿瘤分割的挑战，提出了改进的nnU-Net框架，在BraTS 2025 Task-6验证排行榜上获得第一名。


<details>
  <summary>Details</summary>
Motivation: 儿科脑肿瘤的多参数MRI分割面临数据有限、解剖变异大和机构间成像异质性等独特挑战，需要专门的方法来提高分割准确性。

Method: 使用改进的nnU-Net框架，包括加宽的残差编码器与SE注意力机制、3D深度可分离卷积、特异性驱动的正则化项、小尺度高斯权重初始化，以及两个后处理步骤。

Result: 在Task-6验证排行榜上获得第一名，病灶级别的Dice分数分别为：CC 0.759、ED 0.967、ET 0.826、NET 0.910、TC 0.928、WT 0.928。

Conclusion: 提出的改进nnU-Net框架在儿科高级别胶质瘤分割任务中表现出色，为儿科脑肿瘤的诊断和治疗规划提供了有效的分割工具。
...
### [23] [Investigating Label Bias and Representational Sources of Age-Related Disparities in Medical Segmentation](https://arxiv.org/abs/2511.00477)
*Aditya Parikh,Sneha Das,Aasa Feragen*

Main category: eess.IV

TL;DR: 本文研究了医学图像分割中的算法偏见问题，特别是在乳腺癌分割中对年轻患者的偏见。研究发现偏见主要源于系统性的标签偏见和内在的学习难度差异，而非简单的数据不平衡。


<details>
  <summary>Details</summary>
Motivation: 医学成像中的算法偏见可能加剧健康不平等，但在分割任务中的偏见成因仍未充分理解。尽管分类中的公平性已被广泛研究，但临床重要的分割任务却研究不足。

Method: 通过审计MAMA-MIA数据集建立年龄相关偏见的定量基线，揭示"偏见标尺效应"。进行对照实验系统性地检验偏见来源假设，包括标签质量敏感性和病例难度不平衡。

Result: 发现年轻患者病例本质上更难学习，平衡训练数据无法缓解差异。系统偏见在训练有偏见的机器生成标签时被学习和放大。

Conclusion: 实现公平性需要解决定性的分布差异，而不仅仅是平衡病例数量。这项工作为诊断医学分割中的算法偏见提供了系统框架。
...
### [24] [Image-based ground distance detection for crop-residue-covered soil](https://arxiv.org/abs/2511.00548)
*Baochao Wang,Xingyu Zhang,Qingtao Zong,Alim Pulatov,Shuqi Shang,Dongwei Wang*

Main category: eess.IV

TL;DR: 提出了一种基于图像的方法，通过结合3D相机和RGB相机，在覆盖作物残留物的土壤上精确测量地面距离，解决了传统传感器无法区分残留物和土壤的问题。


<details>
  <summary>Details</summary>
Motivation: 保护性农业中，土壤表面覆盖作物残留物带来好处，但精确控制播种深度的挑战在于缺乏准确的地面距离信息，因为现有传感器无法区分残留物和土壤。

Method: 使用3D相机获取深度图像，RGB相机获取彩色图像；利用彩色图像区分残留物和土壤区域生成掩码图像；将掩码应用于深度图像，仅使用土壤区域深度信息计算地面距离。

Result: 该方法可实现实时实施，测量误差在±3mm范围内，适用于保护性农业机械的精确深度播种。

Conclusion: 该图像方法能有效排除作物残留物干扰，精确测量土壤表面距离，可应用于保护性农业的精密播种及其他需要深度控制的应用场景。
...
### [25] [GDROS: A Geometry-Guided Dense Registration Framework for Optical-SAR Images under Large Geometric Transformations](https://arxiv.org/abs/2511.00598)
*Zixuan Sun,Shuaifeng Zhi,Ruize Li,Jingyuan Xia,Yongxiang Liu,Weidong Jiang*

Main category: eess.IV

TL;DR: 提出GDROS框架，通过几何引导的密集配准方法解决光学-SAR图像配准问题，显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 光学和SAR遥感图像配准是图像融合和视觉导航的关键基础，但由于模态差异（非线性辐射差异、几何畸变、噪声变化）和大几何变换，现有方法难以实现可靠配准

Method: 使用CNN-Transformer混合特征提取模块获取跨模态深度特征，构建多尺度4D相关体积并迭代优化以建立像素级密集对应关系，通过最小二乘回归模块对预测的光流场施加几何约束

Result: 在WHU-Opt-SAR、OS和UBCv2三个代表性数据集上的实验表明，该方法在不同空间分辨率下均表现出鲁棒性能，在所有指标上显著优于当前最先进方法

Conclusion: GDROS框架通过几何引导的密集配准策略有效解决了光学-SAR图像配准中的模态差异和几何变换挑战
...
### [26] [Been There, Scanned That: Nostalgia-Driven LiDAR Compression for Self-Driving Cars](https://arxiv.org/abs/2511.00652)
*Ali Khalid,Jaiaid Mobin,Sumanth Rao Appala,Avinash Maurya,Stephany Berrio Perez,M. Mustafa Rafique,Fawad Ahmad*

Main category: eess.IV

TL;DR: DejaView是一种针对自动驾驶车辆3D点云数据的压缩系统，利用车辆在有限区域内重复行驶的特性，通过查找和利用更大时间尺度（天和月）的冗余性来实现高效压缩。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆每天产生数TB的传感器数据，其中大量是LiDAR等深度传感器产生的3D点云。这些数据需要传输到云端存储，用于机器学习训练或事故调查分析。为了降低网络和存储成本，需要高效的压缩方法。

Method: DejaView基于自动驾驶车辆在有限区域内运行且大多重复相同路线的洞察，设计了核心的diff操作，将点云紧凑地表示为相对于过去3D数据的增量。系统搜索并利用天和月级别的时间尺度冗余性。

Result: 使用两个月的LiDAR数据进行端到端实现，DejaView能够以仅15厘米的重建误差将点云压缩210倍。

Conclusion: DejaView通过利用车辆在有限区域内重复行驶产生的长期时间冗余性，实现了对3D点云数据的高效压缩，显著降低了网络和存储成本。
...
### [27] [Deep Generative Models for Enhanced Vitreous OCT Imaging](https://arxiv.org/abs/2511.00881)
*Simone Sarrocco,Philippe C. Cattin,Peter M. Maloca,Paul Friedrich,Philippe Valmaggia*

Main category: eess.IV

TL;DR: 评估多种深度学习模型用于提升玻璃体OCT图像质量和减少采集时间，发现cDDPM在临床评估中表现最佳，可将采集时间减少四倍。


<details>
  <summary>Details</summary>
Motivation: 提高玻璃体光学相干断层扫描(OCT)的图像质量，同时显著减少图像采集时间，以改善临床诊断效率。

Method: 使用条件去噪扩散概率模型(cDDPMs)、布朗桥扩散模型(BBDMs)、U-Net、Pix2Pix和VQ-GAN等深度学习模型，输入SD ART10图像，输出与通过平均十幅ART10图像获得的伪ART100图像进行比较。

Result: U-Net在PSNR(30.230)和SSIM(0.820)指标上表现最佳，但cDDPM在视觉图灵测试中排名最高(3.07)，达到32.9%的欺骗率和85.7%的解剖结构保持率。在新数据上，cDDPM生成的玻璃体区域比真实ART1或ART10 B扫描更接近ART100参考。

Conclusion: 定量指标与临床评估存在差异，需要综合评估。cDDPM在生成具有临床意义的玻璃体OCT图像方面表现出强大潜力，同时将采集时间减少四倍。
...
### [28] [Evaluating Video Quality Metrics for Neural and Traditional Codecs using 4K/UHD-1 Videos](https://arxiv.org/abs/2511.00969)
*Benjamin Herb,Rakesh Rao Ramachandra Rao,Steve Göring,Alexander Raake*

Main category: eess.IV

TL;DR: 该论文通过主观质量评估研究，比较了传统视频编码器（AV1、VVC）与神经视频编码器（DCVC-FM、DCVC-RT）的性能，并评估了多种质量指标的有效性。


<details>
  <summary>Details</summary>
Motivation: 随着神经视频编码器成为传统压缩方法的有前景替代方案，需要确定现有质量指标是否仍适用于评估其性能。目前缺乏使用精心设计的主观测试来系统研究此问题的研究。

Method: 使用两种传统编码器（AV1、VVC）和两种神经视频编码器变体（DCVC-FM、DCVC-RT），对6个源视频在4种分辨率（360p到2160p）和9个不同QP值下编码，产生216个序列，由30名参与者在受控环境中进行评分。

Result: VMAF和AVQBits|H0|f显示出强Pearson相关性，FasterVQA在无参考指标中表现最佳，PSNR在不同编码器间的序列内比较中显示出最高的Spearman等级相关。传统和神经视频编码器在指标可靠性方面没有显著性能差异。

Conclusion: 研究提供了系统的主观质量评估数据集，表明现有质量指标在评估神经视频编码器时仍然有效，且传统与神经编码器在指标可靠性方面无显著差异。数据集将公开提供。
...
### [29] [Learned Adaptive Kernels for High-Fidelity Image Downscaling](https://arxiv.org/abs/2511.01620)
*Piyush Narhari Pise,Sanjay Ghosh*

Main category: eess.IV

TL;DR: ADK-Net是一种新颖的深度卷积神经网络框架，通过为每个像素和每个颜色通道独立学习空间变化的自适应重采样核，在监督式图像下采样中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 传统图像下采样方法存在模糊或锯齿问题，现有学习方法在考虑通道特异性特征以实现对真实低分辨率图像的最大保真度方面仍有挑战。

Method: 采用分层设计，包含基于ResNet的特征提取器和并行通道特定核生成器，通过L1重建损失进行端到端训练，学习目标下采样变换。

Result: 在包括RealSR数据集在内的标准基准测试中，ADK-Net在PSNR和SSIM指标上相比现有学习方法和传统方法都有显著提升，建立了新的最先进水平。

Conclusion: ADK-Net通过显式处理通道相互依赖性和学习自适应重采样核，在监督式图像下采样中实现了卓越的性能，为高质量图像缩放提供了有效解决方案。
...
### [30] [Model-Free Channel Estimation for Massive MIMO: A Channel Charting-Inspired Approach](https://arxiv.org/abs/2511.00225)
*Pinjun Zheng,Md. Jahangir Hossain,Anas Chaaban*

Main category: eess.SP

TL;DR: 提出了一种结合深度自编码器和LSTM网络的无模型信道估计方法，在massive MIMO系统中通过有限导频实现高效信道估计，相比传统最小二乘法在病态条件下可获得9dB的NMSE提升。


<details>
  <summary>Details</summary>
Motivation: 传统最小二乘法在大规模MIMO系统中需要过高的导频开销，而稀疏估计方法依赖精确的信道模型，在实际中可能不适用。需要开发不依赖精确信道模型的高效估计方法。

Method: 结合深度自编码器和LSTM网络，首先学习保持时间相关性的低维信道表示，然后跟踪这些特征从有限导频中恢复完整信道信息。

Result: 在射线追踪数据集上的仿真结果显示，在病态条件下相比最小二乘法可获得高达9dB的归一化均方误差改进，同时在不同MIMO配置下保持可扩展性。

Conclusion: 该方法提供了一种有效的无模型信道估计解决方案，在大规模MIMO系统中显著降低了导频开销需求，同时保持了良好的估计性能。
...
### [31] [Discrete-Periodic Ambiguity Function of Random Communication Signals](https://arxiv.org/abs/2511.00482)
*Ying Zhang,Fan Liu,Yifeng Xiong,Tao Liu,Shi Jin*

Main category: eess.SP

TL;DR: 本文研究了携带随机数据载荷的通信信号的模糊函数，建立了统一的解析框架来分析ISAC信号的模糊函数特性，证明了归一化期望积分旁瓣水平在所有星座和调制基上具有不变性。


<details>
  <summary>Details</summary>
Motivation: 研究通信信号在ISAC系统中的感知能力，特别是随机数据载荷对模糊函数的影响，这对于理解通信信号在ISAC中的性能至关重要。

Method: 首先开发了统一的解析框架来评估基于任意正交基和i.i.d.星座符号的通信中心ISAC信号的模糊函数，然后推导了离散周期模糊函数及其期望积分旁瓣水平和平均旁瓣水平的闭式表达式。

Result: 证明了归一化期望积分旁瓣水平在所有星座和调制基上具有不变性，这一理论发现通过仿真得到了验证。

Conclusion: 研究为ISAC系统中通信信号的感知性能分析提供了理论基础，揭示了模糊函数特性与调制方案之间的内在关系。
...
### [32] [Meta-Learning Based Radio Frequency Fingerprinting for GNSS Spoofing Detection](https://arxiv.org/abs/2511.00491)
*Leatile Marata,Juhani Sankari,Eslam Eldeeb,Mikko Valkama,Elena Simona Lohan*

Main category: eess.SP

TL;DR: 提出基于元学习的GNSS接收器欺骗信号检测框架，利用射频指纹特征在预相关和后相关阶段检测多种欺骗信号，在TEXBAT和OAKBAT数据集上达到95%以上的检测准确率。


<details>
  <summary>Details</summary>
Motivation: 随着GNSS技术向低地球轨道卫星发展，欺骗干扰信号的安全威胁日益严重，传统数学方法在处理多样化干扰时不够灵活，需要AI驱动的解决方案。

Method: 采用元学习框架，利用GNSS信号在接收器预相关和后相关阶段的射频指纹特征来检测各种类型的欺骗信号。

Result: 在TEXBAT和OAKBAT多个数据集上的数值结果显示，该方法能显著检测不同形式的欺骗信号，检测准确率超过95%，优于现有最先进解决方案。

Conclusion: 提出的元学习框架在GNSS欺骗检测方面具有优越的泛化性能，能够有效应对多种欺骗信号威胁。
...
### [33] [A Multimodal Dataset for Indoor Radio Mapping with 3D Point Clouds and RSSI](https://arxiv.org/abs/2511.00494)
*Ljupcho Milosheski,Kuon Akiyama,Blaž Bertalanič,Jernej Hribar,Ryoichi Shinkuma*

Main category: eess.SP

TL;DR: 本文提出了一个多模态数据集，结合了高分辨率3D激光雷达扫描和Wi-Fi信号强度测量，用于室内无线环境建模研究。


<details>
  <summary>Details</summary>
Motivation: 随着支持带宽密集型应用的智能设备增多，需要可靠的室内无线连接。准确的无线电环境地图估计对于自适应无线网络规划和接入点优化至关重要，但室内空间的复杂性使得生成真实无线电环境地图具有挑战性。

Method: 创建了一个多模态数据集，在20种不同AP配置下收集高分辨率3D LiDAR扫描和Wi-Fi RSSI测量数据，包含无人环境和有人环境两种场景。

Result: 该数据集能够支持研究动态环境对无线信号传播的影响，为数据驱动的无线建模研究提供资源。

Conclusion: 该资源旨在促进数据驱动无线建模研究，特别是在新兴高频标准如Wi-Fi 7的背景下，推动开发稳健、高容量的室内通信系统。
...
### [34] [Fast Time-Varying mmWave Channel Estimation: A Rank-Aware Matrix Completion Approach](https://arxiv.org/abs/2511.00607)
*Tianyu Jiang,Yan Yang,Hongjin Liu,Runyu Han,Bo Ai,Mohsen Guizani*

Main category: eess.SP

TL;DR: 提出了一种用于快速时变毫米波MIMO系统的高维信道估计两阶段压缩感知框架，包括观测矩阵补全和信道矩阵稀疏恢复，通过低秩矩阵补全和秩感知OMP方法实现高效信道估计。
...
Result: 仿真结果表明所提框架有效，在低秩矩阵恢复方面达到最先进性能，并具有理论保证。

Conclusion: 该框架能够实现高精度观测矩阵补全和低延迟稀疏信道恢复，特别适用于处理用户移动性导致的信道秩变化问题。
...
### [35] [Fairness-Aware Secure Communication in ISAC Systems with STAR-RIS and RSMA](https://arxiv.org/abs/2511.00721)
*Thanh Nha To,Hoang Lai Pham,Quynh Nguyen Thi,Tuan Anh Pham,Le Thanh Bang*

Main category: eess.SP

TL;DR: 本文研究了将同时发射和反射可重构智能表面(STAR-RIS)与速率分割多址接入(RSMA)相结合，用于提升集成感知与通信(ISAC)系统中的物理层安全性。


<details>
  <summary>Details</summary>
Motivation: 在集成感知与通信系统中，每个感知目标都可能被视为潜在窃听者，这反映了实际部署条件。需要实现用户间的公平安全通信，同时保持感知性能。

Method: 提出了一种基于交替优化(AO)和主化最小化(MM)方法的高效算法，联合优化基站波束成形向量和STAR-RIS系数。

Result: 仿真结果验证了所提算法的快速收敛性，并展示了安全通信性能的显著提升。

Conclusion: STAR-RIS与RSMA的结合能够有效提升ISAC系统的物理层安全性能，实现公平的安全通信和感知性能保障。
...
### [36] [Target Detection with Tightly-coupled Antennas: Analysis for Unknown Wideband Signals](https://arxiv.org/abs/2511.00779)
*Erfan Khordad,Peter J. Smith,Sachitha C. Bandara,Rajitha Senanayake,Robert W. Heath Jr*

Main category: eess.SP

TL;DR: 本文分析了使用高互耦紧耦合天线阵列进行目标检测的方法，提出针对不同频谱动态特性的检测器，并证明紧耦合天线阵列在目标检测中的优势。


<details>
  <summary>Details</summary>
Motivation: 研究紧耦合天线阵列在目标检测中的应用，利用其宽工作带宽优势，解决互耦导致的噪声频率相关性问题。

Method: 假设目标回波为未知宽带信号，噪声具有频率相关特性。针对回波频谱的恒定、慢变和快变三种动态特性，分别提出最大似然估计检测器和新型慢变检测器。

Result: 仿真验证表明，提出的慢变检测器在频率变化鲁棒性方面优于MLE检测器，紧耦合天线阵列相比弱耦合阵列在目标检测中具有明显优势。

Conclusion: 紧耦合天线阵列在目标检测中具有显著优势，提出的新型检测器能有效处理频率变化问题，为宽带目标检测提供了有效解决方案。
...
### [37] [Stacked Flexible Intelligent Metasurface Design for Multi-User Wireless Communications](https://arxiv.org/abs/2511.00878)
*Ahmed Magbool,Vaibhav Kumar,Marco Di Renzo,Mark F. Flanagan*

Main category: eess.SP

TL;DR: 首次提出基于堆叠柔性智能超表面(SFIM)的通信系统，通过可变形超表面层提升系统性能，相比刚性SIM系统获得显著性能增益


<details>
  <summary>Details</summary>
Motivation: 堆叠智能超表面(SIMs)已成为下一代无线网络的有效解决方案，而柔性智能超表面(FIMs)可通过物理形变增强通信性能，结合两者优势构建更先进的通信系统

Method: 提出SFIM系统模型，包括收发信号模型和信道模型，构建在发射功率预算、形变距离和元原子响应单位模约束下的系统和速率最大化优化问题，采用基于梯度投影法的交替优化框架求解
...
### [38] [Towards Channel Charting Enhancement with Non-Reconfigurable Intelligent Surfaces](https://arxiv.org/abs/2511.00919)
*Mahdi Maleki,Reza Agahzadeh Ayoubi,Marouan Mizmizi,Umberto Spagnolini*

Main category: eess.SP

TL;DR: 本文研究了如何设计全被动电磁表面（EMS）来增强密集城市环境中的信道制图（CC）性能，提出了基于分位数驱动准则的静态EMS相位配置方法，在30 GHz频段的3D射线追踪城市场景中显著降低了定位误差和轨迹中断。


<details>
  <summary>Details</summary>
Motivation: 传统可重构智能表面（RIS）优化仅关注最大化增益，但会抑制位置指纹特征，反而降低信道制图性能。需要解决SNR与空间多样性之间的权衡问题。

Method: 采用两种互补的CC技术（半监督t-SNE和半监督自编码器），设计静态EMS相位配置，通过分位数驱动准则针对最差用户优化，平衡SNR和空间多样性。

Result: 在30 GHz 3D射线追踪城市场景中，提出的EMS将90%分位数定位误差从>50米降低到<25米，严重轨迹中断减少了4倍以上（在15%监督条件下）。

Conclusion: 静态预配置EMS无需重配置开销，可实际支持信道制图，在各种评估配置中均保持一致的性能改进。
...
### [39] [Lightweight ResNet-Based Deep Learning for Photoplethysmography Signal Quality Assessment](https://arxiv.org/abs/2511.00943)
...
TL;DR: 提出了一种轻量级ResNet架构，结合SE模块用于PPG信号质量评估，比较了多种输入配置，在参数和计算量上大幅减少的同时保持了高性能。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习在可穿戴设备中的应用增长，需要在资源受限平台上开发轻量高效的模型，通过不同预处理方法提升性能。

Method: 使用基于ResNet的轻量级深度学习框架，集成SE模块，比较PPG信号及其一阶导数、二阶导数、自相关等多种输入配置。

Result: 在M4M和MIMIC-IV数据集上分别达到96.52%和84.43%的AUC，相比现有研究参数减少99%以上，FLOPs减少60%以上。

Conclusion: 提出的轻量级模型在保持高性能的同时显著降低了计算复杂度，适用于资源受限的可穿戴设备应用。
...
### [40] [Optimizing Uncertainty-Aware Deep Learning for On-the-Edge Murmur Detection in Low-Resource Settings](https://arxiv.org/abs/2511.00966)
*Andrea De Simone,Noemi Giordano,Silvia Seoni,Kristen M. Meiburger,Fabrizio Riente*

Main category: eess.SP

TL;DR: 该研究开发了基于AI的心杂音检测系统，使用三种不同复杂度的CNN架构，结合蒙特卡洛Dropout进行不确定性估计，实现了在边缘设备上高效部署的轻量级模型。


<details>
  <summary>Details</summary>
Motivation: 传统听诊方法主观性强且依赖专家解读，需要开发可靠、自动化的心杂音检测系统，特别是在资源有限的边缘医疗设备上部署。

Method: 使用CirCor心音数据集，比较三种复杂度递增的CNN架构（轻量、基准、重型），应用蒙特卡洛Dropout进行不确定性估计，并评估计算成本和设备部署适用性。

Result: 轻量级模型达到91%准确率，参数数量减少两个数量级；结合不确定性选择性分类使敏感度提高3%，增强了临床可靠性。

Conclusion: 研究证明了在低资源和远程医疗环境中开发计算效率高、不确定性感知的AI心杂音筛查系统的可行性。
...
### [41] [Seed-Induced Uniqueness in Transformer Models: Subspace Alignment Governs Subliminal Transfer](https://arxiv.org/abs/2511.01023)
*Ayşe Selin Okatan,Mustafa İlhan Akbaş,Laxima Niure Kandel,Berker Peköz*

Main category: eess.SP

TL;DR: 研究发现Transformer模型中的潜意识传输主要取决于特质判别子空间的对齐，而非全局表示相似性。相同种子的学生模型会继承这种对齐并显示更高的信息泄漏，而不同种子模型尽管全局表示相似性高，但泄漏显著减少。


<details>
  <summary>Details</summary>
Motivation: 分析Transformer模型中教师嵌入隐藏特征、学生能够线性解码而不影响主任务性能的潜意识传输现象，挑战传统认为传输能力源于全局表示相似性的观点。

Method: 使用具有解耦公共和私有标签的合成语料库，在匹配和独立随机初始化下蒸馏学生模型，采用子空间级CKA诊断和残差化探针分析泄漏机制。

Result: 相同种子学生模型泄漏率τ≈0.24，不同种子学生模型尽管全局CKA>0.9但泄漏率显著降低至τ≈0.12-0.13。安全控制措施（投影惩罚、对抗性反转、正确原因正则化）能减少泄漏而不损害公共任务性能。

Conclusion: 种子诱导的独特性是一种弹性特性，安全多模型部署需要子空间感知的诊断方法，而非依赖全局表示相似性评估。
...
### [42] [On the Performance of Tri-Hybrid Beamforming Using Pinching Antennas](https://arxiv.org/abs/2511.01099)
*Zhenqiao Cheng,Chongjun Ouyang,Nicola Marchetti*

Main category: eess.SP

TL;DR: PASS系统通过优化夹持天线在介质波导上的位置来重构无线信道，结合三混合波束成形技术提升频谱效率和抗路径损耗能力。
...
Method: 提出PASS支持的三混合波束成形系统，优化数字域和模拟域的波束成形器以及夹持天线的位置布局。

Result: 推导了信道容量的闭式上下界，建立了容量随夹持天线数量的缩放规律，数值结果验证了边界的紧密度和性能优势。

Conclusion: PASS三混合波束成形相比传统混合波束成形在相同射频链数量下能获得显著的性能增益。
...
### [43] [Hi-WaveTST: A Hybrid High-Frequency Wavelet-Transformer for Time-Series Classification](https://arxiv.org/abs/2511.01254)
*Huseyin Goksu*

Main category: eess.SP

TL;DR: 本文提出Hi-WaveTST，一种结合时间序列补丁和高频小波特征的混合架构，在UCI-HAR数据集上以93.38%的准确率超越当前最优的PatchTST模型。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer模型如PatchTST虽然性能优异，但忽略了时间序列中重要的高频信息，这些信息对时间动态具有补充作用。

Method: 提出混合架构，在原始时间补丁基础上增加可学习的高频小波特征流，使用深度小波包分解和可学习广义均值池化层提取特征。

Result: 在UCI-HAR数据集上达到93.38% ± 0.0043的平均准确率，显著优于PatchTST基准模型的92.59% ± 0.0039。

Conclusion: 消融研究证明混合架构、深度高频小波分解和可学习GeM池化都是实现最优性能的关键组件。
...
### [44] [Classification of motor faults based on transmission coefficient and reflection coefficient of omni-directional antenna using DCNN](https://arxiv.org/abs/2511.01371)
*Sagar Dutta,Banani Basu,Fazal Ahmed Talukdar*

Main category: eess.SP

TL;DR: 提出一种基于天线的感应电机故障分类方法，使用天线反射系数S11和传输系数S21的频谱图，通过深度卷积神经网络实现高精度分类。
...
Method: 使用天线S11和S21参数的频谱图作为特征，应用深度卷积神经网络进行故障分类。

Result: S11单独分类准确率93%，S21单独98.1%，两者结合达到100%准确率。

Conclusion: 基于天线S参数的方法能有效分类感应电机故障，双参数组合可实现完美分类。
...
### [45] [CRMD: Complex Robust Modal Decomposition](https://arxiv.org/abs/2511.01398)
*Wang Hao,Kuang Zhang,Hou Chengyu,Tan Chenxing,Cui Weiming,Fu Weifeng,Yao Xinran*

Main category: eess.SP

TL;DR: 本文提出了一种复数域的鲁棒模态分解方法，作为原始实数域RMD的自然扩展，在多个真实世界应用中显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 复数信号能更直观地表示真实物理系统的相位信息，在科学和工程领域具有广泛应用价值，因此需要将实数域的鲁棒模态分解扩展到复数域。

Method: 重新推导复数域RMD的数学原理，并开发针对复数域的算法版本。

Result: 在合成仿真数据集和多个真实世界数据集上的实验表明，复数域鲁棒模态分解在各种应用中显著提高了性能。

Conclusion: 复数域鲁棒模态分解是实数域RMD的有效扩展，在毫米波雷达生理信号检测、故障轴承检测、射频无人机识别和WiFi CSI呼吸检测等应用中表现出优越性能。
...
### [46] [MM-2FSK: Multimodal Frequency Shift Keying for Ultra-Efficient and Robust High-Resolution MIMO Radar Imaging](https://arxiv.org/abs/2511.01405)
*Vanessa Wirth,Johanna Bräunig,Martin Vossiek,Tim Weyrich,Marc Stamminger*

Main category: eess.SP

TL;DR: MM-2FSK方法通过结合光学深度传感器提供深度先验，使得毫米波MIMO雷达仅需少量频率即可实现高帧率三维成像，解决了传统方法对大量频率和计算资源的依赖问题。


<details>
  <summary>Details</summary>
Motivation: 传统MIMO雷达在动态场景中面临信号处理算法限制，需要大量频率和计算资源，限制了捕获速率。现有少频率方法需要目标位置先验且深度范围有限。

Method: 提出MM-2FSK方法，结合辅助光学深度传感模态获取深度先验，使毫米波MIMO雷达仅使用少量频率就能实现高帧率捕获。

Result: 在已知地面真实几何形状的各种目标物体上评估，该方法在深度质量方面表现优异，能够与使用大量频率的耗时资源密集型测量相媲美。

Conclusion: 多模态方法通过光学深度先验有效解决了少频率雷达成像的局限性，实现了高质量、高帧率的三维成像。
...
### [47] [AoI-Aware Machine Learning for Constrained Multimodal Sensing-Aided Communications](https://arxiv.org/abs/2511.01406)
*Abolfazl Zakeri,Nhan Thanh Nguyen,Ahmed Alkhateeb,Markku Juntti*

Main category: eess.SP

TL;DR: 提出了一种结合多模态感知和波束预测的框架，在平均感知率约束下通过深度强化学习和神经网络进行波束训练优化，利用信息年龄(AoI)提升感知数据有限时的性能。


<details>
  <summary>Details</summary>
Motivation: 传统波束训练方法开销较大，而环境感知数据可以增强通信波束训练，但实际中由于感知约束或传感器故障，推理时新鲜感知数据的可用性可能受限，需要更现实的多模态感知模型。

Method: 结合深度Q网络(DQN)和神经网络波束预测器，DQN决定感知决策，NN预测最佳波束。引入信息年龄(AoI)到训练中，使用Lyapunov优化设计奖励函数来强制执行平均感知约束。

Result: 在真实世界数据集上的仿真结果显示，在严格感知约束下，AoI感知训练使top-1和top-3推理准确率分别提高了44.16%和52.96%。但随着感知约束放宽，性能增益会减小。

Conclusion: 所提出的AoI感知联合多模态感知和波束预测框架在感知数据受限情况下显著提升了波束预测性能，特别是在严格感知约束条件下效果最为明显。
...
### [48] [Robust Radar Mounting Angle Estimation in Operational Driving Conditions](https://arxiv.org/abs/2511.01431)
*Simin Zhu,Satish Ravindran,Lihui Chen,Alexander Yarovoy,Francesco Fioranelli*

Main category: eess.SP

TL;DR: 提出了一种结合雷达和IMU数据的信号处理流程，用于在真实驾驶场景中准确估计毫米波车载雷达的安装角度，无需受控环境或专用目标。


<details>
  <summary>Details</summary>
Motivation: 研究如何在真实交通条件下准确估计车载雷达的安装角度，克服传统方法需要受控环境、专用目标或特殊驾驶路线的限制。

Method: 使用神经网络处理稀疏噪声雷达测量，排除移动物体检测，估计雷达运动；引入测量模型校正IMU偏差和比例因子误差；基于车辆运动学从雷达运动和横摆率计算安装角度。

Result: 在RadarScenes数据集（超过79公里真实驾驶）上验证，方法在约25秒内获得可靠估计，达到最先进的准确性和鲁棒性。

Conclusion: 首次证明在复杂真实交通条件下无需受控环境即可准确估计车载雷达安装角度，为实际应用提供了可行解决方案。
...
### [49] [Optimizing Movable Antenna Position and Transmissive RIS Phase for Efficient Base Station Design](https://arxiv.org/abs/2511.01575)
...
<details>
  <summary>Details</summary>
Motivation: 可移动天线和透射式可重构智能表面是两种能显著提升无线通信系统灵活性的创新技术，需要探索它们的协同集成以构建更紧凑高效的6G基站。

Method: 提出MA-TRIS集成架构，在近场联合优化天线定位和TRIS相位调整，补偿相位量化损失，即使使用低分辨率相位移相器也能显著增强信号强度。

Result: 天线移动性提供了额外的自由度来增强期望信号并实现更高的信噪比，特别是与TRIS能力结合时。系统性能随TRIS尺寸和天线位置变化而系统提升。
...
### [50] [Clutter Suppression in Bistatic ISAC with Joint Angle and Doppler Estimation](https://arxiv.org/abs/2511.01599)
*M. Ertug Pihtili,Julia Equi,Ossi Kaltiokallio,Jukka Talvitie,Elena Simona Lohan,Ertugrul Basar,Mikko Valkama*

Main category: eess.SP

TL;DR: 提出一种基于2D-rootMUSIC的静态背景杂波抑制算法，用于提升集成感知与通信系统的感知精度
...
Method: 开发了基于二维根多重信号分类的算法来抑制静态背景杂波

Result: 计算机仿真表明该方法能有效抑制强背景杂波，提供准确的参数估计性能，显著提升信杂噪比，优于现有基准方法

Conclusion: 所提出的2D-rootMUSIC算法能够有效解决ISAC系统中的杂波问题，提升感知性能
...
### [51] [AnyPPG: An ECG-Guided PPG Foundation Model Trained on Over 100,000 Hours of Recordings for Holistic Health Profiling](https://arxiv.org/abs/2511.01747)
*Guangkun Nie,Gongzheng Tang,Yujie Xiao,Jun Li,Shun Huang,Deyun Zhang,Qinghao Zhao,Shenda Hong*

Main category: eess.SP

TL;DR: AnyPPG是一个基于PPG信号的基础模型，通过PPG-ECG生理对齐训练，在生理分析和多器官疾病诊断任务中表现出色，展示了PPG作为全面健康评估工具的潜力。


<details>
  <summary>Details</summary>
Motivation: 现有PPG研究受限于标注数据的规模和多样性，限制了模型精度、泛化能力和应用范围。本研究旨在通过基础模型技术探索PPG在整体健康分析中的潜力。

Method: 提出了AnyPPG基础模型，基于大规模多源同步PPG-ECG数据进行预训练，通过将PPG和ECG表征对齐到共享空间，从未标记信号中学习生理学上有意义的特征。

Result: 在11个生理分析任务中，AnyPPG在回归和分类任务上分别比次优模型平均提升12.8%和9.1%。在多器官疾病诊断中，13种疾病AUC超过0.8，137种超过0.7，不仅在心脑血管疾病中表现优异，在帕金森病和慢性肾病等非心血管疾病中也显示出诊断价值。

Conclusion: AnyPPG证明了通过PPG-ECG生理对齐训练的PPG基础模型能够产生准确鲁棒的信号表征，强调了PPG作为系统性多器官健康全面评估工具的潜力。
...
### [52] [On Systematic Performance of 3-D Holographic MIMO: Clarke, Kronecker, and 3GPP Models](https://arxiv.org/abs/2511.01780)
*Quan Gao,Shuai S. A. Yuan,Zhanwen Wang,Wanchen Yang,Chongwen Huang,Xiaoming Chen,Wei E. I. Sha*

Main category: eess.SP

TL;DR: 3D全息MIMO通过体积阵列配置克服了平面阵列的空间相关性和互耦限制，在3GPP城市宏信道中相比传统2D阵列实现了约20%的容量提升。


<details>
  <summary>Details</summary>
Motivation: 传统平面全息MIMO在亚波长间距下存在空间相关性和互耦问题，这限制了有效自由度和信道容量。3D全息MIMO通过利用体积阵列配置来扩大有效孔径并解锁额外的空间模式。

Method: 首次系统评估，联合考虑电磁特性（如互耦和辐射效率）在Clarke、Kronecker和标准3GPP信道模型下分析3D阵列。使用分析推导和全波仿真方法。

Result: 3D架构相比平面基线实现了更高的有效自由度、更窄的波束宽度和显著的容量改进。在0.3λ水平元件间距的3GPP城市宏信道中，3D配置比传统2D阵列容量提升约20%。

Conclusion: 3D全息MIMO在现实条件下展现出鲁棒性和可扩展性，为下一代6G基站阵列设计提供了指导，弥合了理论可行性与实际部署之间的差距。
...
### [53] [Practical Approaches to Quantifying Intra-Pair Skew Impact via Insertion Loss Deviation](https://arxiv.org/abs/2511.01787)
*David Nozadze,Zurab Kiguradze,Amendra Koul,Sayed Ashraf Mamun,Mike Sapozhnikov*

Main category: eess.SP

TL;DR: 提出两个新的互易参数SILD和FOM SILD来量化高速互连中的对内偏斜效应，通过224 Gb/s SerDes测试验证了FOM SILD与误码率的强相关性。


<details>
  <summary>Details</summary>
Motivation: 随着AI工作负载增加和数据中心需求增长，需要超过200 Gb/s的超高速互连。传统时域和频域方法在量化对内偏斜影响时存在可靠性问题，可能导致通道在一个方向满足偏斜规格而在另一个方向不满足。

Method: 引入两个新的互易参数：偏斜引起的插入损耗偏差(SILD)及其互补品质因数(FOM SILD)，使用224 Gb/s SerDes IP和各种不同对内偏斜的通道进行测量。

Result: 测量结果显示FOM SILD与误码率(BER)存在强相关性。当FOM SILD低于0.2-0.3 dB时，BER保持稳定；当超过0.3 dB时，BER明显增加。对3000多条高速双轴电缆的统计分析表明大多数FOM SILD值小于0.1 dB。

Conclusion: 提出的SILD和FOM SILD参数为高速互连评估提供了实用的度量指标，能够有效量化对内偏斜对信号完整性的影响。
