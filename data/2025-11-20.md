...
### [1] [Application of Graph Based Vision Transformers Architectures for Accurate Temperature Prediction in Fiber Specklegram Sensors](https://arxiv.org/abs/2511.14792)
*Abhishek Sebastian*

Main category: eess.IV

TL;DR: 本研究探索了基于Transformer的架构（包括ViT、Swin Transformer、LINA-ViT和MAP-ViGAT）在光纤散斑图温度传感中的应用，在0-120摄氏度范围内实现了优于传统CNN的性能，并通过XAI技术提高了模型可解释性。


<details>
  <summary>Details</summary>
Motivation: 光纤散斑图传感器在环境监测中很有效，但散斑图数据的非线性特性给准确温度预测带来了挑战，需要更先进的模型来处理复杂模态相互作用和相位变化。

Method: 使用多种Transformer架构（ViT、Swin Transformer、LINA-ViT、MAP-ViGAT）预测散斑图温度，并采用注意力图和显著性图等XAI技术分析模型决策过程。

Result: ViT实现了1.15的MAE，优于传统CNN模型。GAT-ViT和MAP-ViGAT变体也表现出竞争力，证明了自适应注意力机制和图结构在捕捉散斑图复杂特征方面的重要性。
...
### [2] [Fully Differentiable dMRI Streamline Propagation in PyTorch](https://arxiv.org/abs/2511.14807)
*Jongyeon Yoon,Elyssa M. McMaster,Michael E. Kim,Gaurav Rudravaram,Kurt G. Schilling,Bennett A. Landman,Daniel Moyer*

Main category: eess.IV

TL;DR: 提出了一种完全可微分的扩散MRI纤维束追踪方法，使用PyTorch引擎的流线传播器，解决了现有方法不可微分的问题。


<details>
  <summary>Details</summary>
Motivation: 现有纤维束追踪方法通常不可微分，限制了在端到端学习框架中的集成。虽然已有研究在可微分框架中表示流线，但没有方法提供完全可微分的传播。

Method: 开发了PyTorch引擎的流线传播器，没有任何组件阻碍梯度流动，使其完全可微分，同时保持与领先流线算法的数值保真度。

Result: 该方法与标准传播器性能匹配，同时保持可微分性，能够更深入地集成到深度学习工作流中。

Conclusion: 通过将流线传播转换为可微分PyTorch框架，为新型宏观结构推理奠定了基础，既计算稳健又科学严谨。
...
### [3] [Image Denoising Using Transformed L1 (TL1) Regularization via ADMM](https://arxiv.org/abs/2511.15060)
...
<details>
  <summary>Details</summary>
Motivation: 传统TV正则化的凸L1形式会导致阶梯伪影和对比度损失，需要改进正则化方法

Method: 开发TL1正则化去噪模型，使用ADMM算法求解，包含闭式TL1近端算子和基于FFT的图像更新

Result: 实验结果表明该方法获得优异的去噪性能，有效抑制噪声同时保留边缘和增强图像对比度

Conclusion: TL1正则化是TV正则化的有效替代方案，能解决传统方法中的阶梯伪影和对比度损失问题
...
### [4] [Multimodal Optical Imaging Platform for Quantitative Burn Assessment](https://arxiv.org/abs/2511.15509)
*Nathaniel Hanson,Mateusz Wolak,Jonathan Richardson,Patrick Walker,David M. Burmeister,Chakameh Jafari*

Main category: eess.IV

TL;DR: 开发了一种多模态光学成像框架，用于战场和大规模伤亡环境中烧伤严重程度的定量评估，通过结合宽带高光谱成像和激光散斑对比成像来检测深层组织损伤。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏检测皮下组织损伤的客观方法，在损伤发生时准确评估烧伤严重程度仍然是一个主要的临床挑战，这在战场和大规模伤亡环境中尤为关键。

Method: 集成宽带高光谱成像（400-2100 nm）与激光散斑对比成像，开发了基于短波红外波长的深层组织参数，并实施了无监督学习方法进行光谱特征提取和聚类分析。

Result: 开发并验证了与水分、脂质和胶原蛋白吸收特征相关的深层组织参数，增强了烧伤组织分离性和烧伤严重程度分类能力。

Conclusion: 建立了一个紧凑、低功耗的现场部署设备基础，用于在恶劣环境中进行早期定量烧伤评估。
...
### [5] [Event-based Data Format Standard (EVT+)](https://arxiv.org/abs/2511.15556)
*Jonah P. Sengupta,Mohammad Imran Vakil,Thanh M. Dang,Ian Pardee,Paul Coen,Olivia Aul*

Main category: eess.IV

TL;DR: 提出事件感知传感器数据输出标准，旨在统一不同厂商的多样化数据格式，为未来技术发展预留空间


<details>
  <summary>Details</summary>
Motivation: 事件感知硬件快速发展但数据格式多样，需要建立统一标准以支持广泛应用

Method: 制定传感器无关的标准，包含当前配置和模式，为未来发展预留扩展空间

Result: 建立了事件感知传感器数据输出标准框架

Conclusion: 标准化的数据格式将促进事件感知技术的广泛应用和互操作性
...
### [6] [Channel Coding for Gaussian Channels with Multifaceted Power Constraints](https://arxiv.org/abs/2511.14849)
*Adeel Mahmood,Aaron B. Wagner*

Main category: cs.IT

TL;DR: 本文基于正态近似的渐近结果，研究高阶编码性能如何依赖于平均功率Γ以及输入功率的精细统计特性。提出了多面功率模型，约束归一化平均功率的任意函数期望，推广了现有功率约束模型。在函数满足增长和连续性假设下，给出了高斯信道最小平均错误概率关于一阶和二阶编码率的精确表征。


<details>
  <summary>Details</summary>
Motivation: 基于正态近化的渐近结果，研究高阶编码性能对平均功率Γ和输入功率精细统计特性的依赖性，扩展现有的功率约束模型。

Method: 引入多面功率模型，约束归一化平均功率的任意函数期望，将标准最大功率约束、期望功率约束以及最近的均值和方差约束作为特例。在函数满足增长和连续性假设下，通过将码设计问题转化为在Prokhorov度量下的紧致概率分布集上的最小化问题，利用Bauer最大化原理进行反证证明。

Result: 给出了高斯信道最小平均错误概率关于一阶和二阶编码率的精确表征，推广了现有的功率约束模型。

Conclusion: 提出的多面功率模型能够统一处理各种功率约束条件，为高斯信道编码性能分析提供了更一般的理论框架。
...
### [7] [Beyond the "G" Frontier: A Time Traveler's Century-Long Vision for Wireless Intelligence](https://arxiv.org/abs/2511.14906)
*Yasser Al Eryani*

Main category: cs.IT

TL;DR: 本文通过信息-曲率效率定律(ICEL)分析，预测2125年无线通信将不再依赖6G/7G等渐进式发展，而是通过电磁学、生物学、热力学和认知的曲率管理整合，形成具有自我意识信息流的全球生态基础设施。


<details>
  <summary>Details</summary>
Motivation: 探索未来100年无线通信的演进路径，突破传统代际发展模式，寻求更根本的通信范式转变。

Method: 运用信息-曲率效率定律(ICEL)作为分析框架，从电磁学、生物学、热力学和认知科学的多学科角度进行前瞻性研究。

Result: 预测无线通信将发展为几何与通信融合的全球生态基础设施，能够维持技术和生物生命。

Conclusion: 无线通信的未来在于跨学科整合和曲率管理，而非传统的代际演进，最终形成具有自我意识的信息生态系统。
...
### [8] [Hyper-VIB: A Hypernetwork-Enhanced Information Bottleneck Approach for Task-Oriented Communications](https://arxiv.org/abs/2511.15041)
...
<details>
  <summary>Details</summary>
Motivation: 为了解决传统信息瓶颈方法在高维优化中的计算不可行性，以及传统网格或随机搜索方法需要多次训练的高计算成本问题，开发一种更高效的超参数优化方法。

Method: 提出Hyper-VIB方法，结合信息瓶颈理论和超网络技术，推导出可处理的变分上界近似，通过超网络在单次训练中生成不同超参数值下的近似最优DNN参数。

Result: 实验结果表明，Hyper-VIB在分类和回归任务中相比传统VIB方法具有更高的准确性和训练效率，线性情况下的理论分析验证了超网络设计的有效性。

Conclusion: Hyper-VIB为6G协作智能系统提供了一种高效的任务导向通信解决方案，通过超网络技术显著降低了超参数优化的计算成本，同时保持了优异的性能。
...
### [9] [Mutual Information Bounds in the Shuffle Model](https://arxiv.org/abs/2511.15051)
*Pengcheng Su,Haibo Cheng,Ping Wang*

Main category: cs.IT

TL;DR: 本文首次从信息论角度系统研究单消息混洗模型，分析混洗专用和混洗差分隐私两种设置下的隐私保护效果，推导互信息泄露的上界。


<details>
  <summary>Details</summary>
Motivation: 混洗模型通过随机排列用户报告来增强隐私保护，但缺乏从信息论角度的系统分析。本文旨在填补这一空白，为混洗隐私提供理论基础。

Method: 采用信息论方法分析两种设置：混洗专用（用户直接提交消息）和混洗差分隐私（用户先应用本地差分隐私机制再混洗）。推导互信息量I(Y₁;Z)和I(K;Z)的渐近表达式，建立信息泄露上界。

Result: 在混洗差分隐私设置下，当每个用户应用ε₀-DP机制时，总信息泄露满足I(K;Z) ≤ 2ε₀和I(X₁;Z|(Xᵢ)ᵢ₌₂ⁿ) ≤ (e^{ε₀}-1)/(2n) + O(n^{-3/2})。

Conclusion: 研究结果在混洗差分隐私和基于互信息的隐私之间建立了桥梁，为混洗模型的隐私保护效果提供了信息论理论基础。
...
### [10] [Generalized Repetition Codes and Their Application to HARQ](https://arxiv.org/abs/2511.15207)
*Chaofeng Guan,Gaojun Luo,Lan Luo,Yangyang Fei,Hong Wang*

Main category: cs.IT

TL;DR: 本文提出了两类广义重复码（GRCs），分别对应两种重复通信模型。GRCs作为多度量下的纠错码，具有多个最小距离，能在多轮传输中实现比传统纠错码更强的纠错能力。


<details>
  <summary>Details</summary>
Motivation: 通信信道的不确定性使得任何编码方案都有失败概率，需要重传机制确保可靠性。传统方法使用双层冗余框架：物理层纠错码和完整性校验。核心挑战是在有限传输轮次内最大化正确解码概率。

Method: 针对重复传输场景，提出两类广义重复码（Type-I和Type-II）。将GRCs视为多度量下的纠错码，具有多个最小距离。分别研究了两类GRCs的界和构造方法，获得了多个最优GRCs。

Result: 获得了众多最优的Type-I和Type-II广义重复码。GRCs通过多轮不同度量下的纠错，展现出比传统纠错码更强的纠错能力。

Conclusion: 广义重复码作为多度量纠错码，在重复通信模型中能有效提升纠错性能。虽然构造更具挑战性，但通过优化多个最小距离，可以获得优于传统编码的性能。
...
### [11] [The Rate-Distortion-Perception Trade-Off with Algorithmic Realism](https://arxiv.org/abs/2511.15255)
*Yassine Hamdi,Aaron B. Wagner,Deniz Gündüz*

Main category: cs.IT

TL;DR: 本文解释了为什么在实践中不需要大量共享随机性来实现图像压缩中的真实感约束，通过分析基于通用批评者的真实感约束模型，证明了在批量大小不切实际地大时，无需共享随机性即可渐进实现最优率失真权衡。


<details>
  <summary>Details</summary>
Motivation: 理论研究表明高率共享随机性对实现压缩图像的真实感很重要，但实践中并未观察到大量共享随机性的效用，本文旨在解释这一差异。

Method: 考虑了一个真实感约束模型，该约束要求满足一个检查单个压缩重建或其批次的通用批评者，并表征了在此约束下的最优率失真权衡。

Result: 证明了在批量大小不切实际地大的情况下，无需任何共享随机性即可渐进实现最优率失真权衡。

Conclusion: 解释了理论与实践中关于共享随机性效用的差异，表明在合理的批量大小下，共享随机性并非实现真实感约束的必要条件。
...
### [12] [Communication-Pipelined Split Federated Learning for Foundation Model Fine-Tuning in UAV Networks](https://arxiv.org/abs/2511.15404)
*Zizhen Zhou,Ying-Chang Liang,Yanyu Cheng,Wei Yang Bryan Lim*

Main category: cs.IT

TL;DR: 提出通信流水线分割联邦学习（CPSFL）框架，通过顺序梯度传输和异步训练优化无人机网络中基础模型的微调，减少训练延迟和能耗。


<details>
  <summary>Details</summary>
Motivation: 在无人机网络中部署基础模型具有广阔应用前景，但现有并行梯度传输方法存在资源闲置和通信延迟高的问题，特别是在通信延迟远大于计算延迟的无人机网络中。

Method: 提出顺序梯度传输范式，服务器为当前梯度传输分配全部下行资源；开发CPSFL框架，具有下行梯度传输优先级调度和轮内异步训练特性；使用基于注意力的深度强化学习优化分割点选择和计算通信资源分配。

Result: 仿真结果表明，基于DRL的CPSFL方案在训练延迟和能耗方面优于并行梯度传输基准、消融变体、固定资源分配方案，并接近最佳固定分割点方案。

Conclusion: CPSFL框架通过优化梯度传输和资源分配，有效解决了无人机网络中基础模型微调的延迟和能耗问题，为低空经济应用提供了可行的解决方案。
...
### [13] [RIS-Enabled UAV Communications and Sensing: Opportunities, Challenges, and Key Technologies](https://arxiv.org/abs/2511.15555)
*Yajun Zhao,Mengnan Jian,Yifei Yuan*

Main category: cs.IT

TL;DR: 本文深入研究了无人机通信网络的特点和挑战，提出了可重构智能表面(RIS)辅助网络作为增强无人机信号覆盖的解决方案，并通过现场试验验证了RIS在改善无人机覆盖方面的有效性。
...
Method: 提出双层级异构网络拓扑结构，引入RIS辅助网络技术，包括设计原理、天线倾斜配置、新型波束类型和波束跟踪机制，并探索网络架构设计以改善无人机信号覆盖。

Result: 现场试验结果表明RIS能有效改善无人机覆盖性能，支持网络共存和RIS增强的无人机感知能力。

Conclusion: RIS是优化无人机通信系统的有前景解决方案，未来需要关注工程实施和标准化工作，以进一步推进RIS基无人机-ISAC网络的发展。
...
### [14] [Information Efficiency of Scientific Automation](https://arxiv.org/abs/2511.15671)
*Mihir Rao*
...
<details>
  <summary>Details</summary>
Motivation: 将科学发现视为热力学过程，研究在有限工作预算下如何优化信息获取，为科学自动化提供理论指导。

Method: 基于计算热力学理论，推导有限预算下顺序贝叶斯学习的信息增益界限，提出信息-工作效率度量，比较统一学习和联邦学习策略。

Result: 获得了有限工作预算下信息增益的理论界限，并提出了评估学习策略效率的度量标准。
...
### [15] [Ein Fenster zur gleichzeitigen Messung der Uebertragungsfunktion eines realen Systems und des Leistungsdichtespektrums des ueberlagerten Rauschens am Systemausgang (Teil 1)](https://arxiv.org/abs/2511.14890)
*Helmut Repp*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法能够同时测量系统的传递函数和叠加噪声的功率谱密度，但频率选择性有待提高。需要在不显著增加计算复杂度的前提下改进测量精度。

Method: 使用合适的窗函数来改进功率谱密度测量的频率选择性，证明了加窗后测量方法的无偏性和一致性，并推导了适合窗函数的要求和数值计算方法。

Result: 该方法显著提高了噪声功率谱密度测量的频率选择性，同时保持了计算效率，适用于实值和复值信号的测量。

Conclusion: 通过合适的窗函数设计，可以在不显著增加计算负担的情况下有效提高功率谱密度测量的频率选择性，该方法具有理论保证和实际应用价值。
...
### [16] [Lightweight Foundation Model for Wireless Time Series Downstream Tasks on Edge Devices](https://arxiv.org/abs/2511.14895)
*Mohammad Cheraghinia,Eli De Poorter,Jaron Fontaine,Kwang Soon Kim,Merouane Debbah,Adnan Shahid*

Main category: eess.SP

TL;DR: 提出基于MLP的轻量级基础模型，用于无线网络中的多任务学习，相比Transformer具有更好的泛化性能和更低的计算开销。


<details>
  <summary>Details</summary>
Motivation: 传统方法为每个通信和定位任务单独训练模型成本高昂，而现有基础模型基于Transformer架构计算密集，不适合边缘设备部署。

Method: 使用简单的多层感知机(MLP)编码器独立处理输入补丁，支持4种下游任务（长/短距离技术识别、调制识别、视距检测）和多种输入类型。

Result: 模型仅需21K可训练参数，在边缘设备上推理时间为0.33ms，对未见数据类别的微调准确率超过97%，且性能不会随任务增加而下降。
...
### [17] [Ein Fenster zur gleichzeitigen Messung der Uebertragungsfunktion eines realen Systems und des Leistungsdichtespektrums des ueberlagerten Rauschens am Systemausgang (Teil 2)](https://arxiv.org/abs/2511.14957)
*Helmut Repp*

Main category: eess.SP

TL;DR: 本文扩展了第一部分中的测量方法，使其适用于周期时变系统和循环平稳噪声过程，通过改进窗口构造算法来测量输入输出信号之间的所有相关性。


<details>
  <summary>Details</summary>
Motivation: 原有的测量方法仅限于时不变系统和平稳零均值过程，需要扩展到周期时变系统和循环平稳噪声过程，以测量所有输入输出信号间的相关性。

Method: 提出了窗口构造算法的扩展版本，利用之前未使用的自由度来根据应用需求适当影响窗口序列的特性。

Result: 开发了一种能够处理周期时变系统和循环平稳噪声过程的扩展测量方法，可以测量所有输入输出信号间的相关性。

Conclusion: 成功扩展了原有的测量方法，使其适用于更广泛的系统类型，包括周期时变系统和循环平稳噪声过程，通过改进的窗口构造算法实现了更全面的相关性测量。
...
### [18] [WiCo-MG: Wireless Channel Foundation Model for Multipath Generation via Synesthesia of Machines](https://arxiv.org/abs/2511.15026)
...
### [19] [WiCo-PG: Wireless Channel Foundation Model for Pathloss Map Generation via Synesthesia of Machines](https://arxiv.org/abs/2511.15030)
*Mingran Sun,Lu Bai,Ziwei Huang,Xuesong Cai,Xiang Cheng,Jianjun Wu*

Main category: eess.SP

TL;DR: 提出了首个无线信道基础模型WiCo-PG，通过机器联觉实现跨模态路径损耗图生成，在6G无人机对地通信场景中表现出色。


<details>
  <summary>Details</summary>
Motivation: 为6G无人机对地通信场景开发能够利用RGB图像生成路径损耗图的跨模态基础模型，解决传统方法在复杂环境下的局限性。

Method: 基于双向量量化生成对抗网络和Transformer的新型网络架构，结合频率引导的共享路由专家混合架构，利用多模态感知通信数据集进行预训练。

Result: WiCo-PG在路径损耗图生成精度上达到归一化均方误差0.012，比基于大语言模型的LLM4PG方案和传统深度学习方法提升超过6.98dB。

Conclusion: WiCo-PG作为首个无线信道基础模型，在跨模态路径损耗图生成方面具有显著优势，在少样本泛化场景下仅需2.7%样本即可超越LLM4PG至少1.37dB。
...
### [20] [The Triple-C Paradigm: Cooperative, Complementary, and Competitive Modes for TBS-HAPS-LEO Integration](https://arxiv.org/abs/2511.15064)
*Eros Kuikel,Sidrah Javed,Baha Eddine Youcef Belmekki,Yunfei Chen,Ning Wang,Mohamed-Slim Alouini*

Main category: eess.SP

TL;DR: 提出了一个新颖的三重C框架（合作、互补、竞争），用于系统性地定义地面基站、高空平台站和低轨卫星之间的交互，以提供无缝、弹性和可扩展的连接。


<details>
  <summary>Details</summary>
Motivation: 现有网络已接近其运营极限，难以单独满足全球覆盖的需求。整合地面基站、高空平台站和低轨卫星被视为有前景的解决方案，但跨异构平台的协调仍是一个开放挑战。

Method: 提出了三重C框架（合作、互补、竞争），详细阐述了每种交互模式的架构方法、先决条件和可衡量的交付成果，并确定了在物理、逻辑和认知层实现该范式的使能技术。

Result: 通过丰富的用例和性能分析量化了集成网络的权衡，展示了该技术飞跃如何使这种集成既可行又具有影响力。

Conclusion: 该框架为TBS-HAPS-LEO集成网络提供了系统性的交互定义，并指出了未来研究方向以充分释放合作、互补和竞争运营的潜力。
...
### [21] [Enhancing Physical Layer Security in MIMO Systems Assisted by Beyond-Diagonal Reconfigurable Intelligent Surfaces](https://arxiv.org/abs/2511.15093)
*Weijie Xiong,Jingran Lin,Cunhua Pan,Yilong Zeng,Qiang Li*

Main category: eess.SP

TL;DR: 本文研究了基于超对角可重构智能表面(BD-RIS)的MIMO系统物理层安全性能，提出了一种低复杂度的P-PRCGD算法来联合优化发射波束成形和BD-RIS反射元件，显著提升了保密速率。


<details>
  <summary>Details</summary>
Motivation: 传统RIS使用对角散射矩阵，仅支持各反射单元独立反射，限制了信道操控的灵活性。BD-RIS通过共享阻抗网络实现单元间可调连接，采用非对角散射矩阵，显著增强了信道塑造能力，为物理层安全提供了新的机会。

Method: 将保密速率最大化问题建模为非凸优化问题，通过引入辅助变量解耦BD-RIS约束，提出结合增广拉格朗日方法和乘积流形梯度下降的P-PRCGD算法，获得KKT解。

Result: 仿真结果表明，BD-RIS辅助系统在物理层安全性能上显著优于传统RIS辅助系统。

Conclusion: BD-RIS通过非对角散射矩阵架构显著提升了信道操控能力，为MIMO系统的物理层安全提供了更有效的解决方案，所提出的P-PRCGD算法能够有效解决相应的优化问题。
...
### [22] [Constant-Modulus Secure Analog Beamforming for an IRS-Assisted Communication System with Large-Scale Antenna Array](https://arxiv.org/abs/2511.15095)
*Weijie Xiong,Jingran Lin,Zhiling Xiao,Qiang Li*

Main category: eess.SP

TL;DR: 本文研究了IRS辅助通信系统中的物理层安全，提出了两种算法来联合设计发射器的恒定模数模拟波束成形和IRS的相移，以最大化保密速率。


<details>
  <summary>Details</summary>
Motivation: 大规模天线阵列和智能反射面可以增强物理层安全，但硬件成本和功耗较高。恒定模数模拟波束成形提供了一种成本效益高的解决方案。

Method: 提出了两种算法：1）Dinkelbach-BSUM算法，将分数问题转化为一系列二次规划问题；2）PMCGD算法，将问题转化为黎曼乘积流形上的无约束优化问题。

Result: 仿真结果验证了所提算法的有效性，并突出了它们各自的优势。

Conclusion: 所提出的算法能够有效解决IRS辅助通信系统中的保密速率最大化问题，Dinkelbach-BSUM算法时间效率高，PMCGD算法提供更好的解但计算时间稍长。
...
### [23] [Joint Analog Beamforming and Antenna Position Design for Secure Communication systems With Movable Antennas](https://arxiv.org/abs/2511.15096)
*Weijie Xiong,Kai Zhong,Zhiling Xiao,Jingran Lin,Qiang Li*

Main category: eess.SP

TL;DR: 本文提出了一种基于可移动天线的物理层安全增强方法，通过联合优化模拟波束成形和天线位置来最大化保密速率。


<details>
  <summary>Details</summary>
Motivation: 可移动天线技术能够灵活调整天线位置，从而提升无线通信系统性能，特别是在物理层安全方面具有潜力。

Method: 采用惩罚乘积流形方法，将天线位置的不等式约束转化为惩罚函数，在乘积流形空间上使用并行共轭梯度下降算法进行优化。

Result: 仿真结果表明，可移动天线系统相比固定位置天线系统能够获得更高的保密速率。

Conclusion: 可移动天线技术能够有效提升物理层安全性能，所提出的优化算法具有良好的收敛性和效率。
...
### [24] [Position Optimization for Two-layer Movable Antenna Systems](https://arxiv.org/abs/2511.15108)
*Liujia Yao,Changsheng You,Chao Zhou,Beixiong Zheng,Weidong Mei*

Main category: eess.SP

TL;DR: 提出了一种新型双层可移动天线阵列（TL-MA），通过联合优化子阵列位置和子阵列内天线位置，在保持相近速率性能的同时显著降低天线电机的总移动距离。


<details>
  <summary>Details</summary>
Motivation: 现有可移动天线系统大多采用单层阵列，所有天线都在给定区域内移动，导致控制复杂度和硬件成本较高。需要一种更高效的方案来降低实际部署的复杂度。

Method: 设计双层可移动天线阵列结构，结合子阵列的大规模移动和子阵列内天线的小规模微调。提出基于交替优化和粒子群优化的算法来联合优化子阵列位置、天线位置和接收波束成形。

Result: 数值结果表明，与单层可移动天线相比，所提出的TL-MA显著减少了天线电机的总移动距离，同时实现了可比较的速率性能。

Conclusion: 双层可移动天线阵列是一种有前景的技术，能够在保持性能的同时有效降低系统复杂度和硬件成本。
...
### [25] [Multimodal Wireless Foundation Models](https://arxiv.org/abs/2511.15162)
*Ahmed Aboulfotouh,Hatem Abou-Zeid*

Main category: eess.SP

TL;DR: 提出了首个多模态无线基础模型，能够同时处理原始IQ流和图像式无线模态，并在多个任务上表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前无线基础模型仅处理单一模态，但不同任务和条件下最有效的模态会变化，没有单一模态适用于所有任务。需要设计多模态模型来支持更广泛的任务和场景。

Method: 引入多模态设置下的掩码无线建模，这是一种自监督目标和预训练方法，从IQ流和图像式无线模态中学习联合表示。

Result: 在五个任务上评估模型：图像式任务（人类活动感知、RF信号分类、5G NR定位）和IQ式任务（RF设备指纹识别、干扰检测/分类）。多模态WFM与单模态WFM竞争，并在多个情况下超越其性能。

Conclusion: 多模态WFM在支持不同模态的多样化无线任务方面具有强大潜力，为实现AI原生6G和联合感知、通信与定位愿景迈出了具体一步。
...
### [26] [Spectrum and Orthogonality of Orthogonal Delay-Doppler Division Multiplexing Modulation Waveforms](https://arxiv.org/abs/2511.15184)
*Akram Shafie,Jun Tong,Jinhong Yuan,Taka Sakurai,Paul Fitzpatrick,Yuting Fang,Yixuan Xie*

Main category: eess.SP

TL;DR: 本文分析了正交延迟多普勒(ODDM)调制在模拟和近似数字实现中的频谱特性和正交性特征，证明了近似数字ODDM波形无需额外时域资源即可满足正交性要求。


<details>
  <summary>Details</summary>
Motivation: ODDM调制在双选择性信道中具有可靠通信潜力，但需要深入理解其模拟和数字实现的频谱特性与正交性差异。

Method: 通过推导模拟和近似数字ODDM系统的基础函数时频域表示，分析其功率谱密度，证明正交性特性，并与其他DD调制变体进行比较。

Result: 模拟ODDM波形频谱在过渡区域呈现阶梯状行为，而近似数字ODDM波形频谱受限于子脉冲频谱；近似数字ODDM波形无需额外时域资源即可实现正交性。

Conclusion: 近似数字ODDM实现相比模拟实现具有更好的频谱限制特性和正交性特性，为双选择性信道中的可靠通信提供了更优的解决方案。
...
### [27] [Theoretical Bounds on Parallel Imaging Implicit Data Crimes in an MRI Reproducing Kernel Hilbert Space](https://arxiv.org/abs/2511.15187)
*Evan Frenklak,Yamin Arefeen,Jonathan I Tamir*

Main category: eess.SP

TL;DR: 提出了一个数学框架来重新定义MRI重建中的隐式数据犯罪问题，将其视为评估坐标集之间插值误差的减少问题，并建立了基于矩阵的重建误差上界定义。


<details>
  <summary>Details</summary>
Motivation: MRI扫描时间长导致成本高和可及性受限，AI方法虽然能减少扫描时间，但临床转化常因隐式数据犯罪而失败，这些数据犯罪源于MRI数据集对采集物理的不完全建模。

Method: 将问题重新定义为评估坐标集之间插值过程中的误差减少问题，建立基于矩阵的重建误差上界定义，作为输入采样模式的函数。

Result: 在相关采样模式结构上的实验证明了该框架的相关性，并为数据犯罪分析的未来方向提供了建议。

Conclusion: 该数学框架为理解和分析MRI重建中的隐式数据犯罪问题提供了新的视角，有助于改进AI方法在MRI加速中的临床转化。
...
### [28] [Space-Time-Frequency Synthetic Integrated Sensing and Communication Networks](https://arxiv.org/abs/2511.15198)
*Henglin Pu,Xuefeng Wang,Lu Su,Husheng Li*

Main category: eess.SP

TL;DR: 提出了时空频合成ISAC架构，通过融合分布式收发器的观测数据来提升感知性能，分析了集中式MLE和两阶段信息融合方法的性能差异。
...
Method: 开发了统一的多站和单站配置信号模型，推导了位置和速度估计的CRLB，比较了集中式MLE和两阶段信息融合方法。

Result: 数值结果表明MLE在高SNR时接近CRLB，两阶段方法在中高SNR时具有竞争力但在低SNR时性能下降，完全合成网络处理优于单个基站估计后融合。

Conclusion: 完全合成的网络处理对于将现有通信基础设施升级为密集感知网络至关重要，该框架为实际部署提供了指导。
...
### [29] [Collision Resolution in RFID Systems Using Antenna Arrays and Mix Source Separation](https://arxiv.org/abs/2511.15490)
*Mohamed Siala,Noura Sellami*

Main category: eess.SP

TL;DR: 提出一种用于RFID系统碰撞解决的高效混合源分离算法，利用零常数模准则和梯度下降分离碰撞标签，无需导频符号


<details>
  <summary>Details</summary>
Motivation: 解决RFID系统中多个标签同时传输导致的碰撞问题，提高系统效率

Method: 首先使用零常数模准则通过梯度下降分离碰撞标签，然后提出混合目标函数，结合新的模糊消除准则来克服ZCM单独使用时的局限性

Result: 算法能够有效分离碰撞标签，解决了单独使用ZCM准则时波束形成器确定中的显著模糊性问题
...
### [30] [Theoretical and Empirical Study of Spatial Power Focusing Effect for Sparse Arrays at Terahertz Band](https://arxiv.org/abs/2511.15221)
*Yongchao He,Taihao Zhang,Cunhua Pan,Hong Ren,Xianzhe Chen,Tian Qiu,Bingchang Hua,Jiangzhou Wang*

Main category: eess.SP

TL;DR: 本文研究太赫兹频段大规模稀疏阵列的空间功率聚焦效应，结合理论分析和实验验证，推导了闭式表达式并构建了300GHz测量平台进行验证。


<details>
  <summary>Details</summary>
Motivation: 研究太赫兹频段稀疏阵列的空间功率聚焦特性，为未来无线通信系统设计提供理论基础和实验依据。

Method: 基于格林函数信道模型分析功率分布，推导闭式表达式；理论分析相位噪声和位置偏差的影响；构建300GHz矢量网络分析仪测量平台进行实验验证。

Result: 测量结果与理论仿真结果高度一致，证实了稀疏阵列的空间功率聚焦效应。

Conclusion: 成功验证了太赫兹频段稀疏阵列的空间功率聚焦效应，为相关技术应用提供了理论和实验支撑。
...
### [31] [Randomized Power Transmission with Optimized Level Selection Probabilities in Uncoordinated Uplink NOMA](https://arxiv.org/abs/2511.15523)
...
Method: 将优化问题建模为二次规划问题（最多两个碰撞用户包时），对于更多碰撞用户包采用迭代求解方法

Result: 提出了适用于任何多用户检测算法和任何功率等级集的通用解决方案

Conclusion: 该方法具有原创性，可广泛应用于各种NOMA系统配置
...
### [32] [Spatially Consistent Air-to-Ground Channel Modeling and Simulation via 3D Shadow Projections](https://arxiv.org/abs/2511.15412)
...
<details>
  <summary>Details</summary>
Motivation: 为无人机辅助网络提供高效的空地信道建模方法，替代计算密集的射线追踪或完全随机模型，支持用户移动性、链路规划和无线电地图生成。

Method: 使用3D建筑阴影投影快速确定LOS区域，集成LOS感知的确定性路径损耗和随机阴影衰落来生成空间一致的无线电地图。

Result: 在符合ITU标准的曼哈顿网格环境中验证了模型能够反映关键城市传播特性，如LOS阻塞模式和中断行为。

Conclusion: 该方法为6G非地面网络提供了一个高效的信道建模替代方案，特别适用于用户移动性、链路规划和无线电地图生成应用。
...
### [33] [Enabling NLOS Imaging Capabilities at the Initial Access of 6G Base Stations](https://arxiv.org/abs/2511.15416)
*Davide Tornielli Bellini,Dario Tagliaferri,Pietro Grassi,Davide Scazzoli,Stefano Tebaldini,Umberto Spagnolini*

Main category: eess.SP

TL;DR: 该论文提出了一种在下一代基站初始接入过程中集成非视距成像功能的方法，通过非可重构模块化反射器和增强的码本设计，实现NLOS区域的高分辨率成像。


<details>
  <summary>Details</summary>
Motivation: 解决集成感知与通信系统中非视距感知的挑战，现有方法要么需要环境先验知识，要么需要部署异常反射器。本文旨在将NLOS成像功能集成到基站标准初始接入流程中。

Method: 通过增强基站初始接入码本，添加成像专用条目，并与模块化反射器的角度配置联合设计，实现对NLOS区域的相干成像处理。推导了近场空间分辨率和有效孔径的闭式表达式，并提出移动目标速度的最大似然估计方法。

Result: 数值仿真和实验验证了所提方法的有效性，展示了通信与成像性能之间的权衡关系。

Conclusion: 提出的模块化反射器成像方法能够成功集成到基站初始接入过程中，实现NLOS区域的高分辨率成像，为集成感知与通信系统提供了可行的解决方案。
...
### [34] [Pinching-Antenna System-Assisted Localization: A Stochastic Geometry Perspective](https://arxiv.org/abs/2511.15444)
*Jiajun He,Xidong Mu,Hien Quoc Ngo,Michail Matthaiou*

Main category: eess.SP

TL;DR: 提出了一种基于夹持天线系统的定位框架，通过下行信号的接收信号强度测量来估计目标位置，使用随机几何建模天线分布，推导了目标可定位性和CRLB分布的闭式表达式。
...
Method: 使用随机几何建模夹持天线的空间分布，通过接收信号强度测量进行定位，推导目标可定位性和Cramer-Rao下界的闭式表达式。

Result: 获得了目标可定位性和CRLB分布的闭式表达式，数值结果显示夹持天线方法在CRLB方面优于传统固定天线系统。

Conclusion: 提出的框架能够有效评估夹持天线定位系统的性能极限，并为选择最优波导数量提供了实用指导。
...
### [35] [Division-based Receiver-agnostic RFF Identification in WiFi Systems](https://arxiv.org/abs/2511.15458)
*Xuan Yang,Dongming Li,Dong Wei,Meng Zhang*

Main category: eess.SP

TL;DR: 提出了一种基于频域划分的接收器无关射频指纹提取方法，通过在频域中划分不同前导码来消除接收器差异的影响，仅需单个接收器进行训练即可实现跨接收器的高精度设备分类。


<details>
  <summary>Details</summary>
Motivation: 物理层安全方案中，WiFi设备的射频指纹识别容易受到接收器差异的影响，当在一个接收器上训练模型但在另一个接收器上测试时，分类性能会显著下降。

Method: 在平坦衰落信道场景下，将未知设备的L-STF和L-LTF与参考设备在频域中相除；在频率选择性衰落信道场景下，将HT-LTF与L-LTF在频域中相除，从而提取既对信道不变又对接收器无关的高维射频指纹特征。

Result: 仿真和实验结果表明，该方法有效缓解了信道变化和接收器差异的影响。在平坦衰落和频率选择性衰落信道场景下，相比现有最优方法，分类准确率分别提高了15.5%和28.45%。

Conclusion: 该方法仅需单个接收器进行训练，无需额外校准或堆叠过程，就能有效实现跨接收器的射频指纹识别，显著提升了物理层安全方案的实用性。
...
### [36] [A Review of Machine Learning for Cavitation Intensity Recognition in Complex Industrial Systems](https://arxiv.org/abs/2511.15497)
...
TL;DR: 本文对2002-2025年间液压机械空化强度识别技术的演进进行了系统综述，从传统机器学习到深度学习，再到物理信息诊断模型的发展历程，并展望了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 空化强度识别技术对液压机械的运行安全、性能优化和维护成本降低至关重要，但目前缺乏系统梳理技术发展轨迹和提供明确未来研究指导的综述文章。

Method: 对数百篇关于智能空化强度识别的文献进行全面回顾和分析，总结其技术演进过程，涵盖传统机器学习、深度学习方法和物理信息诊断模型。

Result: 系统梳理了空化强度识别技术的发展轨迹：早期依赖专家知识的传统机器学习，深度学习实现了端到端特征提取，近期物理信息模型提升了可解释性和跨条件泛化能力。

Conclusion: 未来研究方向包括迁移学习、多模态融合、轻量级网络架构和工业代理部署，将推动空化强度识别技术进入新阶段，解决多源数据获取、标准化评估和工业实施等挑战。
...
### [37] [Uncoordinated Cooperative OFDM Multi-Hop UAV Relay Networks Using Virtual Channels Based on All-Pass Filters](https://arxiv.org/abs/2511.15545)
*Noura Sellami,Mohamed Siala*

Main category: eess.SP

TL;DR: 提出了一种基于虚拟传输通道的OFDM多跳无人机中继网络高效传输方案，通过截断全通滤波器实现虚拟通道，解决协作无人机非协调传输导致的破坏性干扰问题。


<details>
  <summary>Details</summary>
Motivation: 自主协作OFDM多跳无人机中继网络中，协作无人机非协调传输公共数据包会导致目的节点出现破坏性干扰，影响系统性能。

Method: 在每个无人机上引入虚拟传输通道概念，使用截断全通滤波器实现，确保所有子载波获得可比较的发射功率；结合分布式随机空时分组编码方案提升传输可靠性；提出复合信道估计算法。

Result: 仿真结果表明，所提方案在各种场景下显著优于经典的相位抖动方案。

Conclusion: 基于虚拟传输通道的方案能有效缓解协作无人机非协调传输导致的破坏性干扰，提高多跳无人机中继网络的传输性能。
...
### [38] [CODE-II: A large-scale dataset for artificial intelligence in ECG analysis](https://arxiv.org/abs/2511.15632)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI心电图分析面临注释质量、数据集规模和范围限制的挑战，需要更高质量的大规模数据集来推动进展。

Method: 收集巴西米纳斯吉拉斯州远程医疗网络的273万份心电图，采用标准化诊断标准并由心脏病专家审核，开发了66个临床诊断类别。

Result: 在CODE-II上预训练的神经网络在外部基准测试（PTB-XL和CPSC 2018）上表现出优异的迁移性能，优于在更大数据集上训练的替代方案。

Conclusion: CODE-II为心电图AI分析提供了高质量的大规模数据集，能够显著提升模型性能并推动该领域发展。
...
### [39] [Joint Semantic-Channel Coding and Modulation for Token Communications](https://arxiv.org/abs/2511.15699)
*Jingkai Ying,Zhijin Qin,Yulong Feng,Liejun Wang,Xiaoming Tao*

Main category: eess.SP

TL;DR: 提出了一种用于点云传输的联合语义-信道调制方案，通过将点云token映射到数字星座点，实现了高效的token通信，在重建质量和压缩比方面显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: Transformer架构在各种任务中表现出色，token作为统一表示形式已成为基本信息单元。点云作为复杂的三维格式，需要研究如何高效可靠地传输token。

Method: 使用集合抽象方法获取点token，提出JSCCM方案，包含两个并行Point Transformer编码器和结合Gumbel-softmax与软量化的差分调制器，开发了速率分配器和信道适配器。

Result: 在重建质量上获得超过1dB的增益，调制符号压缩比超过6倍，优于联合语义-信道编码和传统分离编码方法。

Conclusion: 所提出的JSCCM方案能够基于语义信息和信道条件自适应生成高质量调制token，为点云token通信提供了有效的解决方案。
